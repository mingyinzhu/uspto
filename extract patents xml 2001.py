#Mingyin Zhu
#before using: compile a list of the zip files you need using "get file names by year.py"

from bs4 import BeautifulSoup as bs
import requests
import zipfile
import json
import pandas as pd
from urllib.request import urlopen
from io import BytesIO
from zipfile import ZipFile
#import xml.etree.ElementTree as ET
import time
import traceback
import lxml.etree as ET
import re

assignee_fintech = ['QWEST COMMUNICATIONS INTERNATIONAL INC.',
'GENERAL MOTORS CORPORATION',
'BUSCH ENTERTAINMENT CORPORATION',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY INC.',
'STORCARD, INC.',
'FIRST DATA CORPORATION',
'AMWAY CORPORATION',
'INTERNATIONAL BUSINESS MACHINES CORPORATION',
'MARKETOCRACY',
'BLACKBIRD HOLDINGS, INC.',
'PITNEY BOWES INCORPORATED',
'DUN  BRADSTREET, INC.',
'INTEL CORPORATION',
'FOLIOFN, INC.',
'TELEFONAKTIEBOLAGET L.M. ERICSSON (PUBL)',
'HAND HELD PRODUCTS, INC.',
'CHASE MANHATTAN BANK',
'XTEC, INCORPORATED',
'VISA U.S.A.',
'IBBOTSON ASSOCIATES, INC.',
'VISA INTERNATIONAL SERVICE ASSOCIATION',
'VIVOTECH, INC.',
'INDIVOS CORPORATION, A DELAWARE CORPORATION',
'CAPITAL ONE FINANCIAL CORPORATION',
'VISA U.S.A., INC.',
'ELLENBY TECHNOLOGIES, INC.',
'MONETAIRE',
'INTERTRUST TECHNOLOGIES CORPORATION',
'FIRST USA BANK, N.A.',
'YAHOO INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY, INC.',
'INNOVATION CONNECTION CORPORATION',
'PROGRESSIVE CASUALTY INSURANCE COMPANY',
'SECURE DATA IN MOTION, INC.',
'ORACLE CORPORATION',
'DISCOVER FINANCIAL SERVICES',
'SONY CORPORATION/SONY ELECTRONICS, INC.',
'BANK ONE, NATIONAL ASSOCIATION',
'TOTAL SYSTEM SERVICES, INC.',
'GIFT CARDS LLC',
'MICROSOFT CORPORATION',
'GE FINANCIAL ASSURANCE HOLDINGS, INC.',
'MOTOROLA, INC.',
'PASSGATE CORPORATION',
'ENDICOTT INTERCONNECT TECHNOLOGIES, INC.',
'AMERICAN EXPRESS TRAVEL RELATED SYSTEMS COMPANY, INC.',
'AMERITRADE HOLDING CORPORATION',
'TEKELEC',
'HUGHES ELECTRONICS CORPORATION',
'E-LOAN, INC.',
'YAHOO! INC.',
'NCR CORPORATION',
'DUN & BRADSTREET, INC.',
'ZIX CORPORATION',
'WACHOVIA CORPORATION',
'BUYERLEVERAGE',
'VISA U.S.A. INC.',
'DIGITAL RIVER, INC.',
'FLORIDA BANKERS ASSOCIATION, INC.',
'ATM EXCHANGE',
'SAFLINK CORPORATION',
'HEWLETT-PACKARD DEVELOPMENT COMPANY, L.P.',
'BANK OF AMERICA CORPORATION',
'SMART CARD INTEGRATORS, INC.',
'BUYERLEVERAGE EMAIL SOLUTIONS LLC',
'R&R CARD SYSTEMS, INC.',
'INDIVOS CORPORATION',
'HITACHI, LTD.',
'GILBARCO INC.',
'PRICEWATERHOUSECOOPERS LLP',
'MORGAN STANLEY',
'MASTERCARD INTERNATIONAL INCORPORATED',
'VISA INTERNATIONAL SERVICE ASSOCIATION, A DELAWARE CORPORATION',
'ARCOT SYSTEMS, INC.',
'JULY SYSTEMS, INC.',
'ECHARGE CORPORATION',
'TYFONE, INC.',
'NORTHWEST AUTO FINANCE CORP.',
'FINANCIAL ENGINES, INC.',
'JOHN H. HARLAND COMPANY',
'ANAKAM L.L.C.',
'DIEBOLD SELF-SERVICE SYSTEMS DIVISION OF DIEBOLD, INCORPORATED',
'ZIH CORP.',
'SBC KNOWLEDGE VENTURES L.P.',
'SIMPLIFI, LLC',
'SBC KNOWLEDGE VENTURES, LP',
'TEXAS INSTRUMENTS, INCORPORATED',
'PAYPAL. INC.',
'FIRST USA BANK, NATIONAL ASSOCIATION',
'AUTHERNATIVE, INC.',
'HSBC NORTH AMERICA HOLDINGS INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY,',
'SCHLUMBERGER OMNES, INC.',
'THE NORTHWESTERN MUTUAL LIFE INSURANCE COMPANY',
'RSA SECURITY INC.',
'J.P. MORGAN ADVISORY SERVICES, INC.',
'DEBIX ONE, INC.',
'PERSPECTIVE PARTNERS',
'KONINKLIJKE PHILIPS ELECTRONICS N.C.',
'HYPERCOM CORPORATION',
'I4 LICENSING LLC',
'EFUNDS CORPORATION',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY, INC., A NEW YORK CORPORATION',
'LOCKHEED MARTIN CORPORATION',
'IMME, LLC',
'SECUREDPAY SOLUTIONS, INC.',
'BELLSOUTH INTELLECTUAL PROPERTY CORPORATION',
'CARDINALCOMMERCE CORPORATION',
'JOVE CORPORATION',
'ROTUBA EXTRUDERS, INC.',
'EBAY INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES CO., INC. A NEW YORK CORPORATION',
'TARGET BRANDS, INC.',
'TRICIPHER, INC.',
'THE HARTFORD STEAM BOILER INSPECTION AND INSURANCE',
'ALOGENT CORPORATION',
'QWEST COMMUNICATIONS INTERNATIONAL INC',
'FIRST ADVANTAGE CORPORATION',
'WESTERN UNION FINANCIAL SERVICES INC.',
'ARTHUR BLANK & COMPANY, INC.',
'SYSTECH CORPORATION',
'FIRST DATA CORPORAITON',
'CATERPILLAR INC.',
'CREDIGY TECHNOLOGIES, INC.',
'LUCENT TECHNOLOGIES INC.',
'INTERSECTIONS, INC.',
'AXIOMA, INC.',
'SWEET SPOT SOLUTIONS, INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES CO., INC.',
'EXPERIAN-SCOREX, LLC',
'VERISIGN, INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES CO., INC., A NEW YORK CORPORATION',
'STMICROELECTRONICS, INC.',
'EARLY WARNING SERVICES, LLC',
'OBOPAY INC.',
'ITG SOFTWARE SOLUTIONS, INC.',
'BROADCOM CORPORATION',
'PHAROS SYSTEMS INTERNATIONAL, INC.',
'BLACKHAWK MARKETING SERVICES, INC',
'BLACKHAWK MARKETING SERVICES, INC.',
'ENCRYPTAKEY, INC.',
'COMPUCREDIT INTELLECTUAL PROPERTY HOLDINGS CORP. II',
'SBC KNOWLEDGE VENTURES, L.P.',
'TOPAZ SYSTEMS, INC.',
'FIRETHORN HOLDINGS, LLC',
'CISCO TECHNOLOGY, INC.',
'FISERV, INC.',
'INTERTRUST TECHNOLOGIES CORP.',
'MOBILE CANDY DISH, INC.',
'CUBIC CORPORATION',
'DISCOVER FINANCIAL SERVICES LLC',
'UTSTARCOM, :INC.',
'ATHENAINVEST, INC.',
'BUYERLEVERAGE E-MAIL SOLUTIONS LLC',
'THE CHASE MANHATTAN BANK',
'WESTERN UNION FINANCIAL SERVICES, INC.',
'VERUS CARD SERVICES',
'AT&T DELAWARE INTELLECTUAL PROPERTY, INC.',
'REVOLUTION MONEY INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY',
'BLACKHAWK NETWORK, INC.',
'THRIVENT FINANCIAL FOR LUTHERANS',
'THE WESTERN UNION COMPANY',
'CINGULAR WIRELESS II, LLC',
'INTERDIGITAL TECHNOLOGY CORPORATION',
'UTSTARCOM, INC.',
'HARTFORD FIRE INSURANCE COMPANY',
'INTERNATIONAL BUSINESS MACHINES CORP.',
'NOVELL, INC.',
'UAT, INC.',
'FINANCIAL CROSSING, INC.',
'FAIR ISAAC CORPORATION',
'ATMEL CORPORATION',
'UNITED STATES CELLULAR CORPORATION',
'RED HAT, INC.',
'VASCO DATA SECURITY INTERNATIONAL, INC.',
'TECHPORCH, INC.',
'MFOUNDRY, INC.',
'VERIZON BUSINESS NETWORK SERVICES, INC.',
'MPOWER MOBILE, INC.',
'QUALCOMM INCORPORATED',
'IS TECHNOLOGIES, LLC',
'BEA SYSTEMS, INC.',
'HUDSON & KEYSE, L.L.C.',
'PHARMA-SMART LLC',
'AUTHENTIUM, INC.',
'GOLDMAN, SACHS & CO',
'F-SQUARED INVESTMENTS, LLC',
'VERIFONE, INC.',
'ORACLE INTERNATIONAL CORPORATION',
'GILBARCO, INC.',
'BASEPOINT ANALYTICS LLC',
'ADVISOR SOFTWARE, INC.',
'EBAY INC',
'SAS INSTITUTE INC.',
'INTUIT INC.',
'LEHMAN BROTHERS INC.',
'TELANON, INC.',
'DIEBOLD, INCORPORATED',
'RESEARCH AFFILIATES, LLC',
'EQUIFAX INC.',
'PROPAY USA, INC.',
'PALMSOURCE, INC.',
'MORNINGSTAR INC.',
'VISA USA, INC.',
'CODEBROKER, LLC',
'INTERNATIONAL BUSINESS MACHINES',
'MASTERCARD INTERNATIONAL, INC.',
'DYNAMICS INC.',
'VALUE CREATION INSTITUTE',
'SYBASE 365, INC',
'DAYLIGHT FORENSIC & ADVISORY LLC',
'AT & T INTELLECTUAL PROPERTY I, L.P.',
'FMR LLC',
'BATTELLE ENERGY ALLIANCE, LLC',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY, INC',
'VERIENT, INC.',
'PLANTRONICS, INC.',
'PERSPECTIVE PARTNERS, LLC',
'FORTENT AMERICAS INC.',
'PROXENSE, LLC',
'VERAYO, INC.',
'BANK ONE, DELAWARE, NATIONAL ASSOCIATION',
'CEDAR RIDGE RESEARCH LLC',
'NOVELL, INC. A DELAWARE CORPORATION',
'IMMERSION CORPORATION',
'BUSINESS LOGIC CORPORATION',
'APPSWARE WIRELESS, LLC',
'SUN MICROSYSTEMS, INC.',
'GENWORTH FINANCIAL,INC',
'OBOPAY, INC.',
'AMERICAN EXPRESSTRAVEL RELATED SERVICES  COMPANY, INC.',
'SEARS BRANDS, L.L.C.',
'OPEN INVENTION NETWORK LLC',
'OPEN INVENTION NETWORK L.L.C',
'EBAY, INC.',
'HSBC TECHNOLOGIES INC.',
'JPMORGAN CHASE BANK, N.A.',
'STMICROELECTRONICS, INC. (A CORPORATION OF THE STATE OF DELAWARE)',
'RETAIL DECISIONS, INC.',
'CAKE FINANCIAL CORPORATION',
'FORTELLIGENT, INC.',
'WELLS FARGO BANK, N.A.',
'DEVICEFIDELITY, INC.',
'AT&T INTELLECTUAL PROPERTY I, L.P.',
'SYBASE, INC.',
'PROPAY, INC.',
'CHOICEPOINT SERVICES, INC',
'SEMTEK INNOVATIVE SOLUTIONS CORPORATION',
'KOUNT INC.',
'RAF TECHNOLOGY, INC.',
'ZILOG, INC.',
'ADAPTIVE PAYMENTS, INC.',
'BANK OF AMERICA',
'APPLE INC.',
'FINANCEWARE, INC.',
'AT&T INTELLECTUAL PROPERTY I, LP,',
'BYALLACCOUNTS, INC.',
'NELNET BUSINESS SOLUTIONS, INC.',
'AT&T MOBILITY II LLC',
'THE PRUDENTIAL INSURANCE COMPANY OF AMERICA',
'RECARGAX, INC.',
'UAT,INC.',
'EQUIFAX, INC.',
'PITNEY BOWES INC.',
'PALM, INC.',
'RISKMETRICS GROUP, INC.',
'MITEK SYSTEMS',
'PALO ALTO RESEARCH CENTER INCORPORATED',
'VERIZON BUSINESS GLOBAL LLC',
'MONEY TREE SOFTWARE',
'DIRECTPOINTE, INC.',
'SECUREDPAY SOLUTIONS, INC., AN ALABAMA CORPORATION',
'GOLDMAN SACHS & CO.',
'SYBASE 365, INC.',
'CLEVX, LLC',
'PAYFORMANCE CORPORATION',
'INTERSECTIONS INC.',
'GENERAL ELECTRIC COMPANY',
'E*TRADE FINANCIAL CORPORATION',
'HELLOWALLET, LLC',
'WALKER DIGITAL, LLC',
'AMERIPRISE FINANCIAL, INC.',
'DYNAMIC CARD SOLUTIONS INTERNATIONAL',
'DYNAMIC SOLUTIONS INTERNATIONAL',
'SALIENT PARTNERS, L.P.',
'HARRIS TECHNOLOGY, LLC',
'ADVANCED MICRO DEVICES, INC.',
'WAUSAU FINANCIAL SYSTEMS, INC.',
'AGERE SYSTEMS INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPNAY, INC.',
'MASTER CARD INTERNATIONAL INCORPORATED',
'MURPHY OIL USA, INC.',
'BBN TECHNOLOGIES CORP.',
'FORTELLIGENT, INC., A DELAWARE CORPORATION',
'PHARMASMART LLC',
'VALIDITY SENSORS, INC.',
'THE DUN AND BRADSTREET CORPORATION',
'VERIZON PATENT & LICENSING INC.',
'MORTGAGESOFT CORPORATION',
'SHIFT4 CORPORATION',
'GM GLOBAL TECHNOLOGY OPERATIONS, INC.',
'SYMBOL TECHNOLOGIES, INC.',
'RECURSION SOFTWARE, INC.',
'X-CARD HOLDINGS, LLC',
'TELCORDIA TECHNOLOGIES, INC.',
'BOKU, INC.',
'MANTAS, INC.',
'SABRE INC.',
'SABRE INC.,',
'FINSPHERE CORPORATION',
'PSI SYSTEMS, INC.',
'TOPAZ SYSTEM, INC.',
'CITICORP DEVELOPMENT CENTER, INC.',
'FIRST AMERICAN CORELOGIC, INC.',
'INTERNATIONAL APPAREL GROUP, LLC',
'BILL.COM, INC.',
'CELLULAR EXPRESS, INC. DBA XIUS-BOGI',
'OTO TECHNOLOGIES, LLC',
'SAFECENTRAL, INC.',
'ENTRUST, INC.',
'EWINWIN, INC.',
'WAL-MART STORES, INC.',
'GOOGLE INC.',
'OUTCAST MEDIA, INC.',
'APRIVA, LLC',
'MERCHANT LINK, LLC',
'CORELOGIC INFORMATION SOLUTIONS, INC.',
'DFS SERVICES LLC',
'VASCO DATA SECURITY, INC.',
'TOKENEX, L.L.C.',
'PRIVASYS, INC.',
'COMPUTER ASSOCIATES THINK, INC.',
'S1 CORPORATION',
'PRINCETON PAYMENT SOLUTIONS',
'SKYCORE LLC,',
'DRESSER, INC.',
'CITIBANK, N.A.',
'SERVE VIRTUAL ENTERPRISES, INC.',
'AOL LLC',
'VISA INTRERNATIONAL SERVICE ASSOCIATION',
'PAYPAL INC.',
'MOCAPAY, INC.',
'REPLYBUY, INC.',
'ROAM DATA INC',
'EDWARD D. JONES & CO., L.P.',
'STRANDS, INC.',
'RAMPART-ID SYSTEMS, INC.',
'CITIBANK DEVELOPMENT CENTER, INC.',
'THE TRAVELERS COMPANIES, INC.',
'SKC&C USA, INC.',
'EXPERIAN MARKETING SOLUTIONS, INC.',
'CHAIN REACTION ECOMMERCE, INC.',
'CELLCO PARTNERSHIP D/B/A VERIZON WIRELESS',
'SYNIVERSE TECHNOLOGIES, INC.',
'VERIZON PATENT AND LICENSING INC.',
'EXODUS PAYMENT SYSTEMS, LLC',
'JPMORGAN CHASE',
'JPMORGAN CHASE, NA',
'COLLECTIVE2 LLC',
'AUTHENTIFY, INC.',
'UNIVERSAL SECURE REGISTRY, LLC',
'FINSPHERE,INC',
'FINSPHERE, INC.',
'FREEDOMPAY INC.',
'EAGLE RIVER HOLDINGS LLC',
'UNISYS CORP.',
'VERIZON PATENT AND LICENSING, INC.',
'SEQUENT SOFTWARE INC.',
'ALLIANZ GLOBAL INVESTORS SOLUTIONS',
'SYMANTEC CORPORATION',
'FINANCIAL ENGINES, INC. A DELAWARE CORPORATION',
'FINANCIAL ENGINES,INC. A DELAWARE CORPORATION',
'RISCONSULTING GROUP LLC, THE',
'BLACKBOARD INC.',
'SIRF TECHNOLOGY INC.',
'BANK OF AMERICAN CORPORATION',
'SONY ELECTRONICS INC.',
'KINGSTON TECHNOLOGY CORPORATION',
'KNOWLEDGEBASE MARKETING, INC., A DELAWARE CORPORATION',
'AMERICAN EXPRESS TRAVEL RATED SERVICES COMPANY, INC.',
'ELECTRONIC PAYMENT EXCHANGE',
'MONEYGRAM INTERNATIONAL, INC.',
'BYTEMARK, INC.',
'DBAY INC.',
'UNITED SERVICES AUTOMOBILE ASSOCIATION',
'ECHOSTAR TECHNOLOGIES L.L.C.',
'ING BANK, FSB (DBA ING DIRECT)',
'METRO ENTERPRISES, INC.',
'NEUBERGER BERMAN FIXED INCOME LLC',
'REV WORLDWIDE, INC.',
'DIGITAL RECOGNITION NETWORK, INC.',
'ONDOT SYSTEMS, INC.',
'TRANS UNION LLC.',
'TRANS UNION LLC',
'DWOLLA CORP.',
'LUMESIS, INC.',
'BOTTOMLINE TECHNOLOGIES (DE) INC.',
'IMPULSESAVE, INC.',
'FIFTH THIRD PROCESSING SOLUTIONS, LLC',
'CARDTRONICS, INC.',
'REM HOLDINGS 3, LLC, A MISSOURI LIMITED LIABILITY COMPANY',
'GOLDMAN, SACHS & CO.',
'ONEID INC.',
'ONEID, INC.',
'EYELOCK INC.',
'CORPORATE BUSINESS SYSTEMS, INC. D/B/A MAGNET CARD SERVICES',
'IPOST',
'EXPERIAN INFORMATION SOLUTIONS, INC.',
'FIRST AMERICAN WAY',
'ZUMIGO, INC.',
'CLAIRMAIL, INC.',
'SECURITY FIRST CORPORATION',
'SCA PROMOTIONS',
'LOYAL3 HOLDINGS, INC.',
'QUALCOMM INNOVATION CENTER, INC.',
'RISK MANAGEMENT SOLUTIONS LLC',
'LOS ALAMOS NATIONAL SECURITY, LLC',
'UNIFUND CORPORATION',
'SALLIE MAE, INC.',
'TRANSACTION NETWORK SERVICES, INC.',
'STRIPE, INC.',
'CITIBANK,',
'CA, INC.',
'THE ACTIVE NETWORK, INC.',
'GLOBALTEL MEDIA, INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICE COMPANY, INC.',
'XEROX CORPORATION',
'SQUARE, INC.',
'MOZIDO, LLC',
'AURUS INC.',
'TFS CAPITAL LLC',
'MIRI SYSTEMS, LLC',
'MAGTEK, INC.',
'STATE FARM MUTUAL AUTOMOBILE INSURANCE COMPANY',
'TIME WARNER CABLE INC.',
'AMERICAN CASH EXCHANGE, INC.',
'PARABIT SYSTEMS, INC.',
'ALLSTATE INSURANCE COMPANY',
'PRISM TECHNOLOGIES LLC',
'ENSERVIO, INC.',
'HSBC FINANCE CORPORATION',
'VMWARE, INC.',
'KEYME, INC.',
'SQUARE INC.',
'AMAZON TECHNOLOGIES, INC.',
'CASHEDGE, INC.',
'OPERA SOLUTIONS, LLC',
'SENSIBLE VISION, INC.',
'RIVERFRONT INVESTMENT GROUP, LLC',
'CLEARXCHANGE, LLC',
'KAARYA LLC',
'PAYDIANT, INC.',
'SMARTYPIG, L.L.C. D.B.A. SOCIAL MONEY',
'JVL VENTURES, LLC',
'JPMORGAN CHASE BANK, N. A.',
'C-SAM, INC.',
'INTERTHINX, INC.',
'PAYMENT PATHWAYS, INC.',
'NETSPECTRUM INC.',
'CORELOGIC SOLUTIONS, LLC',
'FACEBOOK, INC.',
'EFT SOURCE, INC.',
'FLINT MOBILE, INC.',
'CITIGROUP TECHNOLOGY, INC.',
'INSURANCE SERVICES OFFICE, INC.',
'BIZEQUITY LLC',
'WARP 11 HOLDINGS, LLC',
'BARCLAYS BANK DELAWARE',
'PAYNEARME, INC.',
'INSTITUTIONAL CASH DISTRIBUTORS TECHNOLOGY, LLC',
'AUTOSCRIBE CORPORATION',
'GM GLOBAL TECHNOLOGY OPERATIONS LLC',
'SRI INTERNATIONAL',
'GANART TECHNOLOGIES, INC.',
'M-QUBE, INC.',
'MAXIM INTEGRATED PRODUCTS, INC.',
'MORGAN ENTERPRISES OF WESTLAKE, INC.',
'ACCULLINK, INC.',
'SEQUENT SOFTWARE, INC.',
'LENDING TREE, LLC',
'LEXMARK INTERNATIONAL, INC.',
'VOLTAGE SECURITY, INC.',
'JUMIO INC.',
'CA, INC',
'ALLIANZ GLOBAL INVESTORS US LLC',
'CALLSIGN, INC.',
'AGERO, INC.',
'QUISK, INC.',
'TRACFONE WIRELESS, INC.',
'EQUINOX PAYMENTS, LLC',
'ARGOS RISK LLC',
'CAPTIAL ONE FINANCIAL CORPORATION',
'BILL ME LATER, INC.',
'TELESIGN CORPORATION',
'IMS SOFTWARE SERVICES LTD.',
'CITICORP CREDIT SERVICES, INC.',
'MITEK SYSTEMS, INC.',
'CRYPTITE, LLC',
'NXT-ID, INC.',
'AMERIPRISE FINANCIAL CENTER',
'GROUPON, INC.',
'MERCURY PAYMENT SYSTEMS, LLC',
'OPEN INVENTION NETWORK, LLC',
'SUNGARD DATA SYSTEMS INC.',
'FIDELITY NATIONAL E-BANKING SERVICES, INC.',
'BARCLAYS CAPITAL INC.',
'GROUPON ZAPPEDY, INC.',
'BBY SOLUTIONS, INC.',
'SMARTLEAF, INC.',
'BANC OF AMERICA MERCHANT SERVICES, LLC',
'HEARTLAND PAYMENT SYSTEMS, INC.',
'GORDON*HOWARD ASSOCIATES, INC.',
'MASTERCARD INTERNATIONAL INCORPORTED',
'BRAINTREE PAYMENT SOLUTIONS, LLC',
'INVEST N RETIRE LLC',
'E3 RETAIL LLC',
'PANGEA UNIVERSAL HOLDINGS, INC.',
'DIEBOLD SELF-SERVICE SYSTEMS, DIVISION OF DIEBOLD, INCORPORATED',
'UNITED PARCEL SERVICE OF AMERICA, INC.',
'METROPOLITAN LIFE INSURANCE CO.',
'BLACK CARD LLC',
'GALILEO PROCESSING, INC.',
'IGATE TECHNOLOGIES INC.',
'MARSH USA INC.',
'PIETECH, INC.',
'PAYFONE, INC.',
'SECUREDPAY SOLUTIONS, INC,',
'INTERMEC IP CORP.',
'POWERED CARD SOLUTIONS, LLC',
'ZONAR SYSTEMS, INC.',
'KACHINGLE, INC.',
'CIRQUE CORPORATION',
'INDEX SYSTEMS, INC.',
'ZESTFINANCE, INC.',
'RELIANCE COMMUNICATIONS, LLC.',
'EAGENCY, INC.',
'GRAVIC, INC.',
'RAF TECHNOLOGY, INC',
'DWOLLA, INC.',
'AML PARTNERS, LLC',
'24/7 CUSTOMER, INC.',
'EURONET WORLDWIDE, INC.',
'BRIGHTSTAR CORP.',
'DIGIMARC CORPORATION',
'LIFEYIELD, LLC',
'AMERICAN FAMILY MUTUAL INSURANCE COMPANY',
'THE MOTLEY FOOL HOLDINGS, INC.',
'MOBIBUCKS CORP.',
'FANNIE MAE',
'FENNIE MAE',
'DELL PRODUCTS L.P.',
'INDEPENDENCE BANCSHARES, INC.',
'ADVICEWARE',
'JEMSTEP, INC.',
'VIRTUAL ELECTRIC INC.',
'FORD GLOBAL TECHNOLOGIES, LLC',
'CLINKLE CORPORATION',
'BRIVO SYSTEMS, INC.',
'TYFONE, INC',
'LOOKOUT, INC.',
'FEDBID, INC.',
'XOOM CORPORATION',
'C/O EBAY INC.',
'GLOBAL PAYMENTS INC.',
'I-POS SYSTEMS LLC',
'AUTHENTIFY INC.',
'TNT PARTNERS, LLC',
'MICOSOFT CORPORATION',
'KEMESA, INC.',
'AUTHENTIC8, INC.',
'THE DUN & BRADSTREET CORPORATION',
'BIONIC TRADER SYSTEMS, LLC',
'TELECOMMUNICATION SYSTEMS, INC.',
'HOYOS LABS CORP.',
'VEGA LLC',
'HOTHAND, INC.',
'ADOBE SYSTEMS INCORPORATED',
'CHARLES SCHWAB & CO., INC.',
'EBAY INC,',
'ASSURED CONCEPTS GROUP LTD.',
'MERCHANTWAREHOUSE.COM, LLC',
'IDMISSION LLC',
'NUSPAY INTERNATIONAL INCORPORATED',
'AUTOMOTIVEMASTERMIND, LLC',
'1020, INC.',
'EDWARD JONES & CO.',
'PRINCETON PAYMENT SOLUTIONS, LLC',
'SAFETY PAY, INC.',
'AMERICAN EXPRESS TRAVEL RELATED SERVICES COMPANY,  INC.',
'FRAUDFREE FINANCE, LLC',
'UNITED AIRLINES, INC.',
'VERIENT INC.',
'JPMORGAN CHASE BANK, NA',
'CORELOGIC SOLUTIONS LLC',
'OUTERWALL INC.',
'BC INVESTMENT & LEASING, INC.',
'DOUBLE CHECK SOLUTIONS, LLC',
'SCORELOGIX LLC',
'GUARDIAN ANALYTICS, INC.',
'INSTITUTIONAL CASH DISTRIBUTORS, LLC',
'FARMERS GROUP, INC.',
'BRIGHTERION, INC.',
'ACCEPTTO CORPORATION',
'INTHINC TECHNOLOGY SOLUTIONS, INC.',
'OHVA, INC.',
'ACUITY SYSTEMS, INC.',
'1 OAK TECHNOLOGIES, LLC',
'TIPLY, INC.',
'IGT',
'PAYIT, LLC',
'USA TECHNOLOGIES, INC.',
'EVANTIX GRC, LLC',
'MOPHIE, INC.',
'CARD LIMITED, LLC',
'SONAVATION, INC.',
'VANTIV, LLC',
'SAYPAY TECHNOLOGIES, INC.',
'FISERV , INC.',
'RETIREUP, LLC',
'OUTSITE NETWORKS, INC.',
'ALLIANCE DATA SYSTEMS CORPORATION',
'U.S. BANK, NATIONAL ASSOCIATION',
'MAPQUEST, INC.',
'NITRO MOBILE SOLUTIONS, LLC',
'BLACKROCK INDEX SERVICES, LLC',
'CASS INFORMATION SYSTEMS, INC.',
'MASTERCARD INTERNATIONAL CORPORATION',
'TOUCHTUNES MUSIC CORPORATION',
'WAYNE FUELING SYSTEMS LLC',
'GLOBAL ANALYTICS, INC.',
'FREESCALE SEMICONDUCTOR, INC.',
'ZANGULI LLC',
'ALCATEL-LUCENT USA INC.',
'COLLECTIVE DYNAMICS LLC',
'THE MELT',
'ZENDRIVE, INC.',
'AVAYA INC.',
'HARVARD LABEL, INC. DBA HARVARD CARD SYSTEMS',
'MOZIDO, INC.',
'PAYRANGE INC.',
'OPENTABLE, INC.',
'MICROSOFT TECHNOLOGY LICENSING, LLC',
'COIN, INC.',
'HARMAN INTERNATIONAL INDUSTRIES, INCORPORATED',
'GLOBAL CYBERLINK TECHNOLOGIES, LLC',
'UQONTROL, INC.',
'EVERYKEY, LLC',
'PAYWITHMYBANK, INC.',
'INTERACTIVE TKO, INC.',
'INTERACTIVE TKO, INC',
'NOD INC.',
'CAYAN INC.',
'COMENITY LLC',
'CRYPTOGRAPHI, INC.',
'COINBASE, INC.',
'COGNITAS TECHNOLOGIES, INC.',
'BILLEO, INC.',
'SECURUS, LLC',
'MOTOROLA SOLUTIONS, INC',
'C1 BANK',
'IDENTRUST, INC.',
'NATIONAL PAYMENT CARD ASSOCIATION',
'INVENSENSE, INCORPORATED',
'MARIS, LTD.',
'GROUPGIFTING.COM, INC.',
'21, INC.',
'CENTURYLINK INTELLECTUAL PROPERTY LLC',
'PAYPAL, INC.',
'MASTERCARD MOBILE TRANSACTIONS SOLUTIONS, INC.',
'NANTMOBILE, LLC',
'BUY IT MOBILITY NETWORKS INC.',
'MINKASU, INC.',
'THE TRAVELERS INDEMNITY COMPANY',
'ALLIANZ GLOBAL INVESTORS U.S. LLC',
'MASTERCARD INCORPORATED INTERNATIONAL',
'BIOPRINT SECURITY, LTD.',
'GREAT LAKES INCUBATOR, LLC',
'SEQUITUR LABS, INC.',
'ANCHOR ID, INC.',
'KOFAX, INC.',
'INTELLECTUAL VENTURES I LLC',
'SAFANAD INC.',
'UNITED PARCEL SERVICES OF AMERICA, INC.',
'THE TORONTO-DOMINION BANK',
'SEQUITUR LABS INC.',
'PAYDUNK, LLC',
'SSI AMERICA INC.',
'PAYCLIP, INC.',
'INTERCONTINENTAL EXCHANGE HOLDINGS, INC.',
'NCINO, LLC',
'TEXTPOWER, INC.',
'MIDWEST EMPLOYERS CASUALTY COMPANY',
'MASTER CARD INTERNATIONAL INCORPORAION',
'SILOUET, INC.',
'NANTOMICS, LLC',
'ENTERSEKT, LLC',
'WASHINGTON SOFTWARE, INC.',
'BOOM! PAYMENTS, INC.',
'STRATOSPHERE, LLC',
'MODIV MEDIA, INC.',
'CITIGROUP GLOBAL MARKETS, INC.',
'CLOVER NETWORK, INC.',
'CELLTRUST CORPORATION',
'MICROSOFT TECHNOLOGY LICENSING, LLC.',
'MTS HOLDINGS, INC.',
'CAPITAL ONE SERVICES, LLC.',
'CAPITAL ONE SERVICES, LLC',
'GLOBESHERPA INC.',
'BC INVESTMENTS & LEASING, INC.',
'MX TECHNOLOGIES, INC.',
'EBAY ENTERPRISE, INC.',
'QVIVR, INC.',
'BLACKBIRD TECHNOLOGY HOLDINGS, INC.',
'RAISE MARKETPLACE INC.',
'SOCURE INC.',
'EVERYKEY INC',
'CYBER RELIANT CORPORATION',
'ENET IT GROUP, LLC',
'POCKET SYSTEMS, INC.',
'RIPPLE LABS INC.',
'CONNECT FINANCIAL LLC',
'AERENDIR MOBILE INC.',
'WEPAY, INC.',
'LEARNVEST INC.',
'VOLTAGE SECURITY, LLC',
'TONE MOBILE LLC',
'DUO SECURITY, INC.',
'ADTILE TECHNOLOGIES, INC.',
'GYFT, INC.',
'RADIIUS CORP',
'ONEPIN, INC.',
'RIVETZ CORP.',
'CA. INC.',
'FIT PAY, INC.',
'BRANCH BANKING AND TRUST COMPANY',
'NASDAQ, INC.',
'DELL SOFTWARE, INC.',
'MANIFOLD TECHNOLOGY, INC.',
'CULLEN/FROST BANKERS, INC.',
'HUNTINGTON BANCSHARES INCORPORATED',
'SYNAPTICS INCORPORATED',
'CHICAGO MERCANTILE EXCHANGE INC.',
'GDR ACQUISITION COMPANY LLC',
'OMNYPAY INC.',
'E.DIGITAL CORPORATION',
'BOOGOO INTELLECTUAL PROPERTY LLC',
'ENTRUST DATACARD CORPORATION',
'OBERTHUR TECHNOLOGIES OF AMERICA CORP.',
'THE CLEARING HOUSE PAYMENTS COMPANY, L.L.C.',
'CRYPTOGRAPHY RESEARCH, INC.',
'FORTEL ANALYTICS LLC',
'OMNYPAY, INC',
'GEMALTO, INC.',
'BLEU TECH ENTERPRISES, INC.',
'PAY WITH PRIVACY, INC.',
'OBDEDGE, LLC',
'BIS GLOBAL, INC.',
'EZIC INC.',
'ONENETWORKS, INC.',
'XIUS CORP.',
'FITBIT, INC.',
'NOK NOK LABS, INC.',
'ALIGN COMMERCE CORPORATION',
'CAPITAL PREFERENCES, LTD.',
'MASTER INTERNATIONAL INCORPORATED',
'MASTERCARD INTERNATIONAL INCORPORATION',
'ELASTICSEARCH, INC.',
'VESTA CORPORATION',
'DIEBOLD NIXDORF, INCORPORATED',
'AMERICAN FINANCIAL RESOURCES, INC.',
'VIA INTERNATIONAL SERVICE ASSOCIATION',
'THE BANK OF KAUKAUNA',
'GEMALTO INC.',
'ROAM DATA, INC.',
'GENBAND US LLC',
'SIGUE CORPORATION',
'GLOBAL PAYMENTS GAMING SERVICES INC.',
'AINTU INC.',
'SYNEXXUS, INC.',
'ENVIRONMENTAL FINANCIAL PRODUCTS, LLC',
'TEXTILE COMPUTER SYSTEMS, INC.',
'BANK OF  AMERICA CORPORATION',
'CYLANCE INC.',
'TRUSONA, INC.',
'1USF, INC.']
doc_num_fintech = ['20030004827',
'20030004879',
'20030004894',
'20030005289',
'20030005325',
'20030014344',
'20030014357',
'20030014371',
'20030014372',
'20030014633',
'20030014891',
'20030018532',
'20030018587',
'20030019924',
'20030021417',
'20030023543',
'20030028464',
'20030028465',
'20030028466',
'20030028782',
'20030033242',
'20030033257',
'20030033526',
'20030034388',
'20030036989',
'20030037001',
'20030042301',
'20030043485',
'20030046223',
'20030046555',
'20030048890',
'20030050882',
'20030050896',
'20030050899',
'20030051145',
'20030051173',
'20030052168',
'20030055733',
'20030056111',
'20030057274',
'20030057278',
'20030061163',
'20030061168',
'20030061169',
'20030061170',
'20030061171',
'20030061172',
'20030069820',
'20030070078',
'20030070100',
'20030074261',
'20030074568',
'20030078867',
'20030080183',
'20030082504',
'20030085286',
'20030086591',
'20030093348',
'20030093352',
'20030093353',
'20030093360',
'20030093366',
'20030093371',
'20030097324',
'20030097329',
'20030097330',
'20030097344',
'20030101080',
'20030101134',
'20030101137',
'20030101348',
'20030104820',
'20030105707',
'20030105725',
'20030105862',
'20030106935',
'20030110112',
'20030110133',
'20030111527',
'20030115133',
'20030115142',
'20030115151',
'20030115463',
'20030119478',
'20030120554',
'20030120568',
'20030120574',
'20030120575',
'20030120586',
'20030120660',
'20030126020',
'20030126049',
'20030126054',
'20030126062',
'20030126076',
'20030126083',
'20030126094',
'20030126441',
'20030127524',
'20030132284',
'20030132285',
'20030132292',
'20030132293',
'20030132294',
'20030132918',
'20030132974',
'20030135456',
'20030135459',
'20030135463',
'20030135734',
'20030137404',
'20030140004',
'20030141362',
'20030142855',
'20030144952',
'20030144956',
'20030145203',
'20030150915',
'20030154164',
'20030154355',
'20030154375',
'20030159044',
'20030159050',
'20030163413',
'20030163415',
'20030163432',
'20030164851',
'20030167207',
'20030169881',
'20030172018',
'20030172027',
'20030172028',
'20030172040',
'20030172287',
'20030174823',
'20030176931',
'20030179903',
'20030182194',
'20030182241',
'20030182558',
'20030187784',
'20030187787',
'20030188158',
'20030191702',
'20030191715',
'20030191943',
'20030191964',
'20030195842',
'20030195850',
'20030195859',
'20030196106',
'20030200171',
'20030200172',
'20030200184',
'20030205616',
'20030208427',
'20030208432',
'20030208438',
'20030208443',
'20030208449',
'20030208682',
'20030208684',
'20030209608',
'20030210127',
'20030212601',
'20030212622',
'20030212642',
'20030212904',
'20030216997',
'20030218065',
'20030218066',
'20030220841',
'20030220863',
'20030220868',
'20030220886',
'20030221125',
'20030222138',
'20030222152',
'20030222153',
'20030225671',
'20030225703',
'20030229571',
'20030233318',
'20030233325',
'20030233557',
'20030236991',
'20040005051',
'20040006699',
'20040010449',
'20040010695',
'20040011864',
'20040011877',
'20040015430',
'20040015431',
'20040015450',
'20040015451',
'20040019564',
'20040019571',
'20040020982',
'20040024677',
'20040024678',
'20040024679',
'20040024694',
'20040024696',
'20040024710',
'20040026495',
'20040026496',
'20040026506',
'20040029569',
'20040030587',
'20040030601',
'20040030603',
'20040030660',
'20040030667',
'20040030901',
'20040039675',
'20040044615',
'20040044617',
'20040044621',
'20040044739',
'20040046018',
'20040049451',
'20040049455',
'20040049467',
'20040050929',
'20040050930',
'20040054610',
'20040056088',
'20040058705',
'20040059672',
'20040059686',
'20040059688',
'20040059923',
'20040064711',
'20040064728',
'20040067752',
'20040068472',
'20040069845',
'20040070566',
'20040073688',
'20040073792',
'20040073813',
'20040078340',
'20040083111',
'20040087339',
'20040088236',
'20040088247',
'20040088547',
'20040093436',
'20040093496',
'20040094624',
'20040097217',
'20040098328',
'20040099746',
'20040102183',
'20040103049',
'20040103064',
'20040104268',
'20040111322',
'20040111359',
'20040111363',
'20040111367',
'20040112968',
'20040117308',
'20040117317',
'20040117514',
'20040118930',
'20040122768',
'20040122771',
'20040123125',
'20040124246',
'20040127256',
'20040128256',
'20040128508',
'20040128547',
'20040128558',
'20040131997',
'20040133490',
'20040133787',
'20040135801',
'20040138955',
'20040138978',
'20040139008',
'20040139028',
'20040143378',
'20040143550',
'20040143552',
'20040148256',
'20040148286',
'20040149820',
'20040149827',
'20040151323',
'20040153362',
'20040153417',
'20040153715',
'20040155101',
'20040158523',
'20040158525',
'20040158526',
'20040158532',
'20040159700',
'20040167821',
'20040169088',
'20040171406',
'20040172368',
'20040173677',
'20040176134',
'20040177030',
'20040177045',
'20040177047',
'20040181483',
'20040182921',
'20040182922',
'20040193538',
'20040193685',
'20040199448',
'20040199456',
'20040199469',
'20040206815',
'20040210448',
'20040210449',
'20040210498',
'20040210519',
'20040210521',
'20040210531',
'20040215573',
'20040215964',
'20040217178',
'20040218436',
'20040220784',
'20040220838',
'20040220839',
'20040220840',
'20040225557',
'20040225776',
'20040230488',
'20040230489',
'20040230536',
'20040230538',
'20040230807',
'20040232220',
'20040232221',
'20040232222',
'20040232223',
'20040232224',
'20040233037',
'20040233038',
'20040233039',
'20040234117',
'20040235450',
'20040236474',
'20040236475',
'20040236476',
'20040236596',
'20040236624',
'20040236632',
'20040236681',
'20040236699',
'20040236700',
'20040236701',
'20040237360',
'20040238621',
'20040239480',
'20040239481',
'20040242208',
'20040242270',
'20040243514',
'20040243518',
'20040250066',
'20040252012',
'20040254835',
'20040254868',
'20040255131',
'20040257197',
'20040260646',
'20040260656',
'20040260657',
'20040260703',
'20050001028',
'20050001711',
'20050004855',
'20050004856',
'20050005113',
'20050005117',
'20050005118',
'20050005123',
'20050005124',
'20050005133',
'20050005172',
'20050006461',
'20050010510',
'20050010516',
'20050010777',
'20050010786',
'20050015336',
'20050015337',
'20050029344',
'20050029349',
'20050030151',
'20050033653',
'20050033679',
'20050033684',
'20050033686',
'20050033687',
'20050033704',
'20050035200',
'20050035208',
'20050036611',
'20050038701',
'20050038736',
'20050038741',
'20050039027',
'20050039052',
'20050040228',
'20050040242',
'20050043997',
'20050044040',
'20050044373',
'20050049954',
'20050050366',
'20050051619',
'20050052278',
'20050055318',
'20050055562',
'20050055581',
'20050060248',
'20050061884',
'20050067488',
'20050071179',
'20050071282',
'20050075878',
'20050075958',
'20050075985',
'20050076248',
'20050077349',
'20050080677',
'20050080678',
'20050081052',
'20050086160',
'20050086161',
'20050086177',
'20050091151',
'20050091524',
'20050092830',
'20050097033',
'20050097037',
'20050097049',
'20050102209',
'20050102234',
'20050108155',
'20050108522',
'20050108561',
'20050109835',
'20050109838',
'20050114215',
'20050116024',
'20050116810',
'20050119961',
'20050119968',
'20050119969',
'20050122209',
'20050127164',
'20050131790',
'20050131813',
'20050131815',
'20050131816',
'20050131826',
'20050133606',
'20050137757',
'20050137953',
'20050137969',
'20050137977',
'20050137986',
'20050138362',
'20050138374',
'20050139685',
'20050144108',
'20050144110',
'20050144244',
'20050144452',
'20050149455',
'20050154662',
'20050154665',
'20050154671',
'20050154676',
'20050154923',
'20050156026',
'20050157873',
'20050160050',
'20050165667',
'20050165684',
'20050165695',
'20050169504',
'20050171882',
'20050171905',
'20050171908',
'20050172137',
'20050177484',
'20050177499',
'20050177500',
'20050177501',
'20050177502',
'20050177503',
'20050178825',
'20050178829',
'20050178830',
'20050180618',
'20050182538',
'20050182712',
'20050182735',
'20050184166',
'20050187883',
'20050187901',
'20050189416',
'20050192884',
'20050192885',
'20050192899',
'20050194438',
'20050194450',
'20050197954',
'20050198176',
'20050203765',
'20050203841',
'20050203843',
'20050203856',
'20050204129',
'20050205662',
'20050209975',
'20050210239',
'20050210240',
'20050211766',
'20050211784',
'20050212657',
'20050212658',
'20050222929',
'20050228731',
'20050232471',
'20050234697',
'20050234761',
'20050234762',
'20050238174',
'20050240522',
'20050242195',
'20050246260',
'20050246278',
'20050246290',
'20050249383',
'20050250538',
'20050256803',
'20050257409',
'20050258245',
'20050261972',
'20050261984',
'20050262002',
'20050267842',
'20050268107',
'20050268110',
'20050269402',
'20050273442',
'20050273626',
'20050278249',
'20050278255',
'20050279827',
'20050283830',
'20060000891',
'20060000892',
'20060000893',
'20060000894',
'20060000895',
'20060000896',
'20060000897',
'20060000898',
'20060000899',
'20060000900',
'20060004653',
'20060004659',
'20060006224',
'20060006225',
'20060010060',
'20060015358',
'20060015452',
'20060015453',
'20060015733',
'20060015743',
'20060016868',
'20060016869',
'20060016870',
'20060016871',
'20060016872',
'20060016873',
'20060016874',
'20060016875',
'20060016876',
'20060016877',
'20060016884',
'20060018450',
'20060020479',
'20060020558',
'20060022039',
'20060023694',
'20060026099',
'20060026108',
'20060029261',
'20060031149',
'20060031161',
'20060031173',
'20060032906',
'20060032908',
'20060032909',
'20060041507',
'20060043170',
'20060043173',
'20060043180',
'20060049255',
'20060049256',
'20060054688',
'20060054698',
'20060064363',
'20060064373',
'20060064378',
'20060064380',
'20060064391',
'20060065748',
'20060069643',
'20060069921',
'20060069922',
'20060074698',
'20060074788',
'20060074813',
'20060085357',
'20060085844',
'20060090203',
'20060091223',
'20060095351',
'20060095363',
'20060095528',
'20060097040',
'20060101509',
'20060102717',
'20060102729',
'20060106515',
'20060106738',
'20060107049',
'20060107052',
'20060107067',
'20060111817',
'20060118621',
'20060118624',
'20060122749',
'20060122902',
'20060122931',
'20060123465',
'20060124756',
'20060129485',
'20060131385',
'20060131389',
'20060131390',
'20060131396',
'20060131410',
'20060136317',
'20060136332',
'20060136717',
'20060143071',
'20060143101',
'20060143119',
'20060144924',
'20060144925',
'20060144927',
'20060147000',
'20060149651',
'20060149971',
'20060155639',
'20060157553',
'20060161435',
'20060161789',
'20060163345',
'20060163353',
'20060165060',
'20060167823',
'20060169767',
'20060174104',
'20060175405',
'20060178982',
'20060179007',
'20060179312',
'20060180660',
'20060186209',
'20060190372',
'20060190383',
'20060192006',
'20060195398',
'20060196929',
'20060200410',
'20060200427',
'20060200855',
'20060202025',
'20060204048',
'20060204051',
'20060206403',
'20060210082',
'20060212376',
'20060212384',
'20060212386',
'20060212407',
'20060213970',
'20060213971',
'20060213972',
'20060213973',
'20060215886',
'20060218068',
'20060218096',
'20060218097',
'20060218098',
'20060224899',
'20060224901',
'20060226216',
'20060226224',
'20060226235',
'20060229961',
'20060235761',
'20060235795',
'20060235796',
'20060236395',
'20060237529',
'20060237530',
'20060237531',
'20060237533',
'20060239512',
'20060239513',
'20060241923',
'20060242039',
'20060242046',
'20060242047',
'20060242048',
'20060242049',
'20060242050',
'20060242051',
'20060248005',
'20060248018',
'20060248020',
'20060249574',
'20060253364',
'20060253389',
'20060255128',
'20060258397',
'20060261149',
'20060261174',
'20060265339',
'20060265602',
'20060266822',
'20060269061',
'20060271496',
'20060273163',
'20060277144',
'20060277411',
'20060278697',
'20060278698',
'20060282363',
'20060282381',
'20060282660',
'20060283957',
'20060287004',
'20060287963',
'20060287964',
'20060291703',
'20060293989',
'20060293990',
'20060294023',
'20070001001',
'20070005445',
'20070005500',
'20070005510',
'20070005511',
'20070005514',
'20070007334',
'20070011066',
'20070011079',
'20070011099',
'20070011100',
'20070011103',
'20070011104',
'20070012757',
'20070012763',
'20070016500',
'20070016542',
'20070016796',
'20070016943',
'20070017972',
'20070022025',
'20070023531',
'20070027726',
'20070033121',
'20070033123',
'20070033136',
'20070033139',
'20070033148',
'20070033642',
'20070038546',
'20070038568',
'20070038581',
'20070038867',
'20070043656',
'20070043662',
'20070045394',
'20070045398',
'20070050291',
'20070050840',
'20070053306',
'20070054677',
'20070055621',
'20070055672',
'20070055684',
'20070055732',
'20070055785',
'20070057037',
'20070057038',
'20070057052',
'20070057070',
'20070061237',
'20070061257',
'20070061258',
'20070061259',
'20070061273',
'20070061873',
'20070063025',
'20070067206',
'20070067207',
'20070067208',
'20070067209',
'20070067634',
'20070067833',
'20070069013',
'20070073618',
'20070073619',
'20070073621',
'20070073630',
'20070078741',
'20070078780',
'20070078781',
'20070080211',
'20070082678',
'20070083444',
'20070088950',
'20070089164',
'20070089168',
'20070090182',
'20070094097',
'20070094124',
'20070094135',
'20070094150',
'20070094152',
'20070094154',
'20070095892',
'20070098175',
'20070100719',
'20070100751',
'20070100752',
'20070100754',
'20070106582',
'20070107044',
'20070108269',
'20070108271',
'20070110282',
'20070112688',
'20070119918',
'20070119919',
'20070119920',
'20070119921',
'20070124211',
'20070124235',
'20070124236',
'20070124238',
'20070124589',
'20070124810',
'20070130043',
'20070130070',
'20070130085',
'20070130462',
'20070130463',
'20070131759',
'20070132548',
'20070136115',
'20070136189',
'20070136192',
'20070136211',
'20070136573',
'20070136586',
'20070175987',
'20070177768',
'20070178883',
'20070179903',
'20070179908',
'20070180263',
'20070181670',
'20070181671',
'20070185820',
'20070186099',
'20070187482',
'20070187516',
'20070190861',
'20070192165',
'20070192240',
'20070192245',
'20070192247',
'20070194104',
'20070194123',
'20070198361',
'20070198362',
'20070198410',
'20070198435',
'20070198436',
'20070203732',
'20070203833',
'20070203836',
'20070205270',
'20070205290',
'20070208600',
'20070208640',
'20070208671',
'20070210161',
'20070214076',
'20070214085',
'20070214093',
'20070215688',
'20070215689',
'20070215693',
'20070215698',
'20070219882',
'20070219901',
'20070219905',
'20070219926',
'20070219928',
'20070220272',
'20070221726',
'20070223685',
'20070223706',
'20070226114',
'20070226129',
'20070226130',
'20070228154',
'20070228161',
'20070233614',
'20070233615',
'20070239572',
'20070239586',
'20070239591',
'20070239604',
'20070239622',
'20070241182',
'20070241183',
'20070244811',
'20070244831',
'20070245148',
'20070245157',
'20070245158',
'20070246533',
'20070250393',
'20070250427',
'20070250920',
'20070251994',
'20070251995',
'20070251999',
'20070252009',
'20070254712',
'20070255620',
'20070255638',
'20070255645',
'20070255652',
'20070255653',
'20070255657',
'20070255662',
'20070257100',
'20070257103',
'20070257104',
'20070257105',
'20070257107',
'20070257767',
'20070260544',
'20070260558',
'20070260883',
'20070262134',
'20070262135',
'20070265807',
'20070265945',
'20070265946',
'20070265947',
'20070265958',
'20070265964',
'20070265984',
'20070266257',
'20070271200',
'20070271342',
'20070271596',
'20070276765',
'20070278290',
'20070280509',
'20070280510',
'20070282638',
'20070282728',
'20070282729',
'20070282730',
'20070282754',
'20070284434',
'20070284435',
'20070288320',
'20070288346',
'20070288355',
'20070288394',
'20070288641',
'20070288758',
'20070289000',
'20070290034',
'20070291936',
'20070291995',
'20070291996',
'20070294182',
'20070294195',
'20070295804',
'20070296544',
'20070296551',
'20070299781',
'20070299782',
'20070299783',
'20080004915',
'20080005037',
'20080005576',
'20080006685',
'20080006688',
'20080006691',
'20080008359',
'20080008363',
'20080010161',
'20080010181',
'20080010190',
'20080010191',
'20080010192',
'20080010193',
'20080010196',
'20080010204',
'20080010214',
'20080010215',
'20080010217',
'20080010218',
'20080010220',
'20080010470',
'20080011823',
'20080011825',
'20080011830',
'20080011831',
'20080011838',
'20080011850',
'20080013796',
'20080013807',
'20080015941',
'20080015990',
'20080015992',
'20080015993',
'20080015994',
'20080016002',
'20080016003',
'20080016005',
'20080016010',
'20080017704',
'20080017721',
'20080021840',
'20080027815',
'20080029593',
'20080029598',
'20080029607',
'20080033722',
'20080033767',
'20080033852',
'20080034209',
'20080034221',
'20080035738',
'20080040265',
'20080040271',
'20080040276',
'20080040284',
'20080040285',
'20080046334',
'20080046366',
'20080046379',
'20080046723',
'20080046747',
'20080046986',
'20080048024',
'20080048025',
'20080051059',
'20080051122',
'20080052183',
'20080052192',
'20080052232',
'20080052233',
'20080052235',
'20080052245',
'20080054068',
'20080054079',
'20080054081',
'20080055048',
'20080059364',
'20080059366',
'20080059367',
'20080059369',
'20080059375',
'20080064442',
'20080065524',
'20080065531',
'20080065553',
'20080065554',
'20080065555',
'20080067242',
'20080067247',
'20080071680',
'20080071681',
'20080071682',
'20080071702',
'20080071876',
'20080072065',
'20080077417',
'20080077462',
'20080077526',
'20080078832',
'20080082452',
'20080085698',
'20080086342',
'20080086402',
'20080091545',
'20080091602',
'20080091617',
'20080093467',
'20080097905',
'20080097925',
'20080102766',
'20080102790',
'20080103980',
'20080103982',
'20080103985',
'20080104709',
'20080109357',
'20080109657',
'20080110977',
'20080110978',
'20080110980',
'20080110982',
'20080110983',
'20080114696',
'20080114703',
'20080116285',
'20080119212',
'20080120195',
'20080120214',
'20080120229',
'20080120236',
'20080120507',
'20080120509',
'20080120717',
'20080121687',
'20080126145',
'20080126252',
'20080126259',
'20080126260',
'20080133373',
'20080133390',
'20080133403',
'20080133410',
'20080133412',
'20080133419',
'20080133420',
'20080133421',
'20080133422',
'20080140564',
'20080140575',
'20080140576',
'20080140580',
'20080146248',
'20080147544',
'20080147546',
'20080147565',
'20080148059',
'20080148394',
'20080149718',
'20080149727',
'20080149728',
'20080154734',
'20080154735',
'20080154757',
'20080154760',
'20080154766',
'20080154770',
'20080154772',
'20080154792',
'20080155655',
'20080156870',
'20080162312',
'20080162338',
'20080162346',
'20080162349',
'20080162361',
'20080162362',
'20080164307',
'20080164308',
'20080167000',
'20080167017',
'20080167961',
'20080167967',
'20080167980',
'20080167991',
'20080168006',
'20080169345',
'20080169351',
'20080170693',
'20080170776',
'20080172316',
'20080172317',
'20080172730',
'20080172735',
'20080173708',
'20080173721',
'20080177661',
'20080177662',
'20080181408',
'20080182592',
'20080183508',
'20080183590',
'20080183629',
'20080183874',
'20080185429',
'20080189142',
'20080189214',
'20080195425',
'20080195444',
'20080195445',
'20080195499',
'20080195545',
'20080195550',
'20080197201',
'20080199053',
'20080200144',
'20080201226',
'20080201264',
'20080203151',
'20080203152',
'20080203156',
'20080203170',
'20080203172',
'20080207203',
'20080207234',
'20080208697',
'20080208739',
'20080208741',
'20080208742',
'20080208743',
'20080208744',
'20080208746',
'20080208758',
'20080208759',
'20080208762',
'20080209223',
'20080210754',
'20080211623',
'20080215501',
'20080215636',
'20080217395',
'20080221934',
'20080221947',
'20080221970',
'20080221971',
'20080221972',
'20080221973',
'20080221990',
'20080222015',
'20080222016',
'20080222027',
'20080222048',
'20080222049',
'20080222720',
'20080223918',
'20080223932',
'20080223937',
'20080223939',
'20080227391',
'20080227471',
'20080228538',
'20080228539',
'20080228540',
'20080228541',
'20080228556',
'20080228606',
'20080228635',
'20080228648',
'20080229108',
'20080229384',
'20080229398',
'20080229410',
'20080230598',
'20080230614',
'20080235136',
'20080235154',
'20080235513',
'20080236991',
'20080238610',
'20080242274',
'20080243558',
'20080243680',
'20080243690',
'20080243701',
'20080243703',
'20080243713',
'20080243716',
'20080245855',
'20080245856',
'20080245857',
'20080249908',
'20080249909',
'20080249910',
'20080249924',
'20080249925',
'20080249927',
'20080249928',
'20080249929',
'20080249930',
'20080249933',
'20080249939',
'20080249947',
'20080252487',
'20080255722',
'20080255897',
'20080255947',
'20080255993',
'20080257959',
'20080258890',
'20080262670',
'20080262935',
'20080262973',
'20080265018',
'20080268811',
'20080270301',
'20080270302',
'20080275814',
'20080275818',
'20080277482',
'20080281744',
'20080281925',
'20080283590',
'20080283591',
'20080283592',
'20080283593',
'20080288377',
'20080288385',
'20080288403',
'20080288404',
'20080288405',
'20080289030',
'20080294540',
'20080294554',
'20080294690',
'20080296367',
'20080298588',
'20080300895',
'20080301056',
'20080301445',
'20080301461',
'20080302869',
'20080302876',
'20080306850',
'20080306876',
'20080308625',
'20080308627',
'20080308629',
'20080313064',
'20080313079',
'20080313082',
'20080313097',
'20080313707',
'20080313720',
'20080314971',
'20080314995',
'20080317220',
'20080319801',
'20080319802',
'20080319887',
'20080319897',
'20080319901',
'20080319904',
'20080319909',
'20080319914',
'20090006232',
'20090006262',
'20090006263',
'20090006264',
'20090006269',
'20090007249',
'20090008462',
'20090012901',
'20090018924',
'20090018940',
'20090018954',
'20090018969',
'20090018982',
'20090024273',
'20090024419',
'20090024523',
'20090024529',
'20090024540',
'20090024851',
'20090024881',
'20090030740',
'20090030819',
'20090030842',
'20090030843',
'20090030844',
'20090030845',
'20090030864',
'20090031407',
'20090036164',
'20090037284',
'20090037285',
'20090037308',
'20090037321',
'20090037322',
'20090037323',
'20090037344',
'20090037982',
'20090039153',
'20090043706',
'20090044020',
'20090044266',
'20090045251',
'20090045960',
'20090048953',
'20090048957',
'20090048967',
'20090048971',
'20090048974',
'20090048979',
'20090049534',
'20090051510',
'20090055277',
'20090055314',
'20090055316',
'20090055323',
'20090055327',
'20090055893',
'20090055907',
'20090057393',
'20090060199',
'20090061831',
'20090063312',
'20090063330',
'20090063345',
'20090063363',
'20090077646',
'20090077675',
'20090078755',
'20090078764',
'20090078765',
'20090079539',
'20090083160',
'20090083184',
'20090083191',
'20090083544',
'20090084840',
'20090085761',
'20090089122',
'20090089210',
'20090089211',
'20090089213',
'20090089869',
'20090090783',
'20090094123',
'20090094125',
'20090094126',
'20090094149',
'20090094155',
'20090094158',
'20090094171',
'20090099959',
'20090099960',
'20090099975',
'20090099978',
'20090100168',
'20090101720',
'20090103725',
'20090104888',
'20090106118',
'20090106136',
'20090106152',
'20090106153',
'20090106160',
'20090106178',
'20090108061',
'20090108064',
'20090109037',
'20090112649',
'20090112709',
'20090112745',
'20090112752',
'20090112765',
'20090112767',
'20090112768',
'20090112774',
'20090112775',
'20090114718',
'20090117883',
'20090119159',
'20090119170',
'20090119184',
'20090119190',
'20090119194',
'20090119205',
'20090119207',
'20090119212',
'20090119213',
'20090119228',
'20090121012',
'20090121016',
'20090124234',
'20090125430',
'20090125439',
'20090125446',
'20090125466',
'20090132271',
'20090132290',
'20090132293',
'20090132299',
'20090132347',
'20090132362',
'20090132404',
'20090132417',
'20090132423',
'20090132424',
'20090132425',
'20090138406',
'20090138722',
'20090138953',
'20090140044',
'20090144160',
'20090144161',
'20090144185',
'20090144193',
'20090144202',
'20090145972',
'20090150248',
'20090150295',
'20090150312',
'20090150667',
'20090150994',
'20090152343',
'20090152366',
'20090156180',
'20090157454',
'20090157528',
'20090157554',
'20090157556',
'20090157557',
'20090157560',
'20090158029',
'20090158038',
'20090158404',
'20090159680',
'20090159696',
'20090159702',
'20090159703',
'20090159709',
'20090160617',
'20090164256',
'20090164373',
'20090164380',
'20090164381',
'20090166435',
'20090171756',
'20090171757',
'20090171837',
'20090171845',
'20090171849',
'20090171852',
'20090172770',
'20090172795',
'20090173782',
'20090173784',
'20090173790',
'20090177576',
'20090177589',
'20090177591',
'20090177612',
'20090177892',
'20090181644',
'20090182653',
'20090182673',
'20090182674',
'20090182676',
'20090183249',
'20090184164',
'20090184167',
'20090185241',
'20090185687',
'20090185736',
'20090185737',
'20090185738',
'20090187492',
'20090187507',
'20090187508',
'20090187759',
'20090191811',
'20090192855',
'20090192875',
'20090192876',
'20090192903',
'20090192907',
'20090192912',
'20090192916',
'20090192932',
'20090192935',
'20090192937',
'20090192944',
'20090192947',
'20090192957',
'20090193500',
'20090198610',
'20090198611',
'20090198616',
'20090198632',
'20090198633',
'20090199004',
'20090199264',
'20090199272',
'20090200371',
'20090200385',
'20090202081',
'20090204457',
'20090204815',
'20090210308',
'20090210344',
'20090210349',
'20090216676',
'20090216680',
'20090216681',
'20090217034',
'20090218392',
'20090222308',
'20090222369',
'20090222373',
'20090222374',
'20090222375',
'20090222376',
'20090222379',
'20090222380',
'20090222383',
'20090224060',
'20090228364',
'20090228365',
'20090228370',
'20090230195',
'20090233577',
'20090235346',
'20090235563',
'20090240594',
'20090240598',
'20090243821',
'20090248465',
'20090248559',
'20090248560',
'20090248567',
'20090248568',
'20090248569',
'20090248570',
'20090248571',
'20090248572',
'20090248573',
'20090248581',
'20090248583',
'20090248589',
'20090248600',
'20090249082',
'20090249478',
'20090250523',
'20090254440',
'20090254448',
'20090254463',
'20090254479',
'20090254480',
'20090254485',
'20090254491',
'20090254981',
'20090255996',
'20090259580',
'20090259588',
'20090259848',
'20090261161',
'20090261162',
'20090261166',
'20090265252',
'20090265272',
'20090265275',
'20090265281',
'20090266884',
'20090270045',
'20090271276',
'20090271322',
'20090271327',
'20090271343',
'20090271617',
'20090271629',
'20090271853',
'20090271856',
'20090275363',
'20090276347',
'20090276356',
'20090276372',
'20090278660',
'20090281904',
'20090281944',
'20090281948',
'20090281949',
'20090281958',
'20090281959',
'20090283586',
'20090287512',
'20090287536',
'20090287601',
'20090287929',
'20090287932',
'20090288143',
'20090289105',
'20090289112',
'20090292641',
'20090294524',
'20090294526',
'20090294527',
'20090298540',
'20090299759',
'20090299896',
'20090299911',
'20090300742',
'20090300746',
'20090300747',
'20090300749',
'20090307139',
'20090307140',
'20090307142',
'20090307778',
'20090308921',
'20090310778',
'20090313069',
'20090313132',
'20090313134',
'20090313163',
'20090313168',
'20090319287',
'20090319290',
'20090319420',
'20090319421',
'20090319425',
'20090319426',
'20090319435',
'20090319784',
'20090321519',
'20090322477',
'20090325542',
'20090326972',
'20090327010',
'20090327099',
'20090327114',
'20090327128',
'20090327131',
'20090327132',
'20090327155',
'20090328179',
'20090328184',
'20090328207',
'20100004981',
'20100005013',
'20100005035',
'20100005303',
'20100006641',
'20100006642',
'20100010878',
'20100010906',
'20100010935',
'20100012715',
'20100012721',
'20100014720',
'20100017859',
'20100020946',
'20100020971',
'20100022254',
'20100023434',
'20100023437',
'20100023449',
'20100023450',
'20100023453',
'20100024017',
'20100025462',
'20100029306',
'20100030586',
'20100030588',
'20100030633',
'20100030688',
'20100030689',
'20100030693',
'20100030695',
'20100030697',
'20100030699',
'20100031021',
'20100032480',
'20100034434',
'20100036749',
'20100040293',
'20100041368',
'20100042454',
'20100042526',
'20100042538',
'20100042539',
'20100042547',
'20100043266',
'20100048247',
'20100049615',
'20100049658',
'20100050233',
'20100051684',
'20100051685',
'20100051691',
'20100051705',
'20100057509',
'20100057552',
'20100057614',
'20100057619',
'20100057620',
'20100057622',
'20100057623',
'20100057624',
'20100058068',
'20100061294',
'20100062746',
'20100062758',
'20100063867',
'20100063889',
'20100063895',
'20100063897',
'20100063922',
'20100063924',
'20100063926',
'20100063935',
'20100063945',
'20100064361',
'20100065629',
'20100070405',
'20100070418',
'20100070419',
'20100070757',
'20100071031',
'20100075630',
'20100075638',
'20100076891',
'20100077466',
'20100078471',
'20100078472',
'20100078473',
'20100082384',
'20100082466',
'20100082467',
'20100082481',
'20100082486',
'20100082490',
'20100082501',
'20100084476',
'20100088188',
'20100088210',
'20100088228',
'20100088231',
'20100088237',
'20100088753',
'20100089998',
'20100090006',
'20100094742',
'20100094752',
'20100094753',
'20100094755',
'20100094768',
'20100097180',
'20100100486',
'20100100502',
'20100102122',
'20100106638',
'20100107229',
'20100108762',
'20100114634',
'20100114681',
'20100114740',
'20100114775',
'20100114776',
'20100114778',
'20100120408',
'20100121778',
'20100121779',
'20100121780',
'20100121786',
'20100122316',
'20100123002',
'20100123003',
'20100125509',
'20100125510',
'20100125516',
'20100125522',
'20100127083',
'20100131300',
'20100131303',
'20100131304',
'20100131305',
'20100131307',
'20100131308',
'20100131375',
'20100131390',
'20100131397',
'20100131413',
'20100132043',
'20100138309',
'20100138312',
'20100138327',
'20100138341',
'20100138344',
'20100140347',
'20100145836',
'20100145847',
'20100145854',
'20100145858',
'20100145860',
'20100145861',
'20100145868',
'20100145875',
'20100146605',
'20100150424',
'20100153011',
'20100153269',
'20100153271',
'20100153272',
'20100153274',
'20100153307',
'20100153451',
'20100155470',
'20100156596',
'20100161403',
'20100161468',
'20100161470',
'20100161494',
'20100169182',
'20100169202',
'20100169237',
'20100170946',
'20100174566',
'20100174596',
'20100174611',
'20100174646',
'20100174647',
'20100175126',
'20100176193',
'20100179902',
'20100179909',
'20100179920',
'20100180326',
'20100180327',
'20100185466',
'20100185545',
'20100185559',
'20100191602',
'20100191622',
'20100191633',
'20100191645',
'20100191649',
'20100191653',
'20100191960',
'20100198722',
'20100198728',
'20100201484',
'20100205108',
'20100205110',
'20100205448',
'20100208889',
'20100211498',
'20100211503',
'20100211504',
'20100211507',
'20100211528',
'20100214058',
'20100217708',
'20100217709',
'20100217999',
'20100223145',
'20100223146',
'20100223168',
'20100223186',
'20100223199',
'20100224683',
'20100228580',
'20100228638',
'20100228668',
'20100228685',
'20100228831',
'20100229245',
'20100230487',
'20100235282',
'20100235298',
'20100235299',
'20100237153',
'20100241464',
'20100241465',
'20100241535',
'20100241567',
'20100241570',
'20100241850',
'20100242104',
'20100243728',
'20100243732',
'20100243741',
'20100245037',
'20100248710',
'20100250364',
'20100250390',
'20100250411',
'20100250434',
'20100250436',
'20100250441',
'20100250442',
'20100250469',
'20100251343',
'20100252623',
'20100252624',
'20100254537',
'20100256996',
'20100257066',
'20100257081',
'20100257357',
'20100258625',
'20100260388',
'20100262542',
'20100262546',
'20100280893',
'20100280910',
'20100280914',
'20100280927',
'20100280935',
'20100280943',
'20100280944',
'20100280950',
'20100280955',
'20100280976',
'20100287086',
'20100287097',
'20100287100',
'20100287250',
'20100293093',
'20100293094',
'20100293099',
'20100293100',
'20100293101',
'20100293381',
'20100293382',
'20100299239',
'20100299243',
'20100299256',
'20100299259',
'20100299265',
'20100299267',
'20100301114',
'20100305993',
'20100306029',
'20100306032',
'20100306076',
'20100306081',
'20100306095',
'20100306105',
'20100306107',
'20100306113',
'20100306126',
'20100306127',
'20100306531',
'20100306819',
'20100308109',
'20100308110',
'20100308117',
'20100312667',
'20100312698',
'20100312709',
'20100312717',
'20100313245',
'20100314446',
'20100315678',
'20100318468',
'20100320266',
'20100323660',
'20100325041',
'20100325045',
'20100325047',
'20100325052',
'20100325053',
'20100325062',
'20100325438',
'20110000960',
'20110004526',
'20110004547',
'20110004550',
'20110004553',
'20110004557',
'20110004569',
'20110004921',
'20110004933',
'20110006113',
'20110010234',
'20110010292',
'20110010293',
'20110010296',
'20110016050',
'20110016051',
'20110016052',
'20110016053',
'20110017820',
'20110022421',
'20110022472',
'20110022481',
'20110022483',
'20110022513',
'20110022515',
'20110022517',
'20110022518',
'20110022522',
'20110022541',
'20110024496',
'20110024508',
'20110029428',
'20110031317',
'20110035294',
'20110035317',
'20110035318',
'20110035333',
'20110035604',
'20110035788',
'20110040581',
'20110040640',
'20110040686',
'20110041170',
'20110047036',
'20110047040',
'20110047044',
'20110047045',
'20110047071',
'20110047074',
'20110047075',
'20110047602',
'20110047608',
'20110053644',
'20110055013',
'20110055057',
'20110055058',
'20110055076',
'20110055077',
'20110055110',
'20110055115',
'20110060684',
'20110062235',
'20110065420',
'20110066454',
'20110066493',
'20110066505',
'20110066516',
'20110066550',
'20110066551',
'20110066553',
'20110068168',
'20110071914',
'20110071945',
'20110071949',
'20110071956',
'20110072279',
'20110073644',
'20110073663',
'20110073665',
'20110076941',
'20110077052',
'20110078021',
'20110078031',
'20110078073',
'20110078081',
'20110078083',
'20110079645',
'20110082791',
'20110082793',
'20110082802',
'20110082814',
'20110084131',
'20110084139',
'20110084140',
'20110084147',
'20110087573',
'20110087574',
'20110087596',
'20110087617',
'20110087622',
'20110091092',
'20110093322',
'20110093383',
'20110093397',
'20110098023',
'20110099079',
'20110099086',
'20110099108',
'20110099112',
'20110099113',
'20110099384',
'20110101109',
'20110105022',
'20110106659',
'20110106682',
'20110106691',
'20110106693',
'20110106701',
'20110106702',
'20110106710',
'20110108623',
'20110112950',
'20110112954',
'20110112958',
'20110112985',
'20110113248',
'20110113487',
'20110114717',
'20110117966',
'20110119106',
'20110119155',
'20110119190',
'20110119733',
'20110121071',
'20110125597',
'20110125624',
'20110125632',
'20110125638',
'20110131104',
'20110131110',
'20110131122',
'20110131130',
'20110131131',
'20110131151',
'20110136429',
'20110137748',
'20110137801',
'20110137802',
'20110137803',
'20110137804',
'20110139869',
'20110140834',
'20110140841',
'20110142234',
'20110143710',
'20110145082',
'20110145122',
'20110145148',
'20110145152',
'20110145167',
'20110145901',
'20110147451',
'20110153461',
'20110153496',
'20110153498',
'20110153499',
'20110153503',
'20110154447',
'20110154466',
'20110154467',
'20110155801',
'20110155802',
'20110161100',
'20110161118',
'20110161188',
'20110161231',
'20110161232',
'20110161233',
'20110161671',
'20110166869',
'20110166923',
'20110166979',
'20110166994',
'20110166999',
'20110167004',
'20110173077',
'20110173116',
'20110173121',
'20110173122',
'20110173124',
'20110173684',
'20110174879',
'20110178883',
'20110178925',
'20110178926',
'20110178928',
'20110178954',
'20110179290',
'20110179494',
'20110180601',
'20110184820',
'20110184851',
'20110184867',
'20110184868',
'20110184884',
'20110187505',
'20110187642',
'20110191125',
'20110191156',
'20110191160',
'20110191161',
'20110191237',
'20110191244',
'20110191251',
'20110191252',
'20110191262',
'20110191263',
'20110194750',
'20110196753',
'20110196771',
'20110196782',
'20110196783',
'20110196786',
'20110196791',
'20110196792',
'20110196797',
'20110196818',
'20110197074',
'20110197265',
'20110198395',
'20110202417',
'20110202453',
'20110202463',
'20110202465',
'20110202978',
'20110204140',
'20110208652',
'20110208653',
'20110208657',
'20110209222',
'20110211689',
'20110213628',
'20110213653',
'20110213711',
'20110213807',
'20110218827',
'20110218856',
'20110218860',
'20110218877',
'20110218880',
'20110218911',
'20110218915',
'20110220726',
'20110223972',
'20110225057',
'20110225076',
'20110225089',
'20110225090',
'20110225091',
'20110225094',
'20110226854',
'20110226858',
'20110231284',
'20110231318',
'20110231332',
'20110231911',
'20110237223',
'20110237224',
'20110237296',
'20110238473',
'20110238511',
'20110238512',
'20110238514',
'20110238517',
'20110238550',
'20110238559',
'20110238564',
'20110238571',
'20110238578',
'20110238579',
'20110238580',
'20110238581',
'20110239272',
'20110240745',
'20110240748',
'20110244920',
'20110246315',
'20110246316',
'20110246317',
'20110246362',
'20110246372',
'20110246385',
'20110247062',
'20110247063',
'20110250866',
'20110251869',
'20110251892',
'20110251906',
'20110251910',
'20110251913',
'20110251945',
'20110251946',
'20110251948',
'20110251951',
'20110251954',
'20110251962',
'20110251965',
'20110258075',
'20110258117',
'20110258120',
'20110258123',
'20110258142',
'20110258452',
'20110258686',
'20110264543',
'20110264558',
'20110264583',
'20110266340',
'20110266354',
'20110267205',
'20110269438',
'20110270694',
'20110270741',
'20110270757',
'20110270779',
'20110271106',
'20110271114',
'20110272468',
'20110272469',
'20110272471',
'20110272472',
'20110272474',
'20110272478',
'20110272479',
'20110272481',
'20110276418',
'20110276424',
'20110276425',
'20110276465',
'20110276468',
'20110276469',
'20110276470',
'20110276471',
'20110276487',
'20110276492',
'20110276494',
'20110276495',
'20110276511',
'20110278357',
'20110282778',
'20110282779',
'20110282785',
'20110282788',
'20110282806',
'20110284632',
'20110287726',
'20110287808',
'20110288965',
'20110288989',
'20110288993',
'20110289004',
'20110289006',
'20110289017',
'20110295733',
'20110295748',
'20110295753',
'20110295768',
'20110300802',
'20110302014',
'20110302019',
'20110302067',
'20110302085',
'20110302089',
'20110307381',
'20110307388',
'20110307710',
'20110309142',
'20110312270',
'20110313835',
'20110313870',
'20110313872',
'20110313897',
'20110313898',
'20110313900',
'20110313921',
'20110313922',
'20110313925',
'20110314529',
'20110314539',
'20110315781',
'20110320290',
'20110320293',
'20110320314',
'20110320343',
'20110320344',
'20110320347',
'20110320351',
'20110321173',
'20120002095',
'20120004949',
'20120005039',
'20120005053',
'20120005096',
'20120005733',
'20120006891',
'20120010930',
'20120011007',
'20120011009',
'20120011011',
'20120011012',
'20120011024',
'20120011062',
'20120011066',
'20120011070',
'20120011565',
'20120012653',
'20120016731',
'20120016795',
'20120016797',
'20120016812',
'20120017089',
'20120017269',
'20120018506',
'20120018511',
'20120019361',
'20120019379',
'20120022945',
'20120023026',
'20120023567',
'20120024945',
'20120024950',
'20120028609',
'20120028612',
'20120029945',
'20120029999',
'20120030043',
'20120030044',
'20120030048',
'20120030078',
'20120030082',
'20120030083',
'20120030096',
'20120030098',
'20120030142',
'20120031969',
'20120032782',
'20120035980',
'20120036001',
'20120036013',
'20120036038',
'20120036042',
'20120036065',
'20120039469',
'20120041877',
'20120041881',
'20120046975',
'20120046979',
'20120047064',
'20120047070',
'20120047075',
'20120050179',
'20120052800',
'20120054046',
'20120054090',
'20120054101',
'20120054102',
'20120054121',
'20120054498',
'20120056971',
'20120059741',
'20120059751',
'20120059758',
'20120059762',
'20120061466',
'20120063601',
'20120066024',
'20120066065',
'20120066080',
'20120066108',
'20120066122',
'20120066124',
'20120066129',
'20120066130',
'20120066131',
'20120066501',
'20120066740',
'20120067949',
'20120067964',
'20120071205',
'20120072243',
'20120072244',
'20120072247',
'20120072311',
'20120072334',
'20120072343',
'20120072344',
'20120072345',
'20120072350',
'20120072354',
'20120072714',
'20120074217',
'20120074232',
'20120078735',
'20120078751',
'20120078788',
'20120078789',
'20120078791',
'20120078792',
'20120078799',
'20120078812',
'20120079581',
'20120082306',
'20120084103',
'20120084177',
'20120084203',
'20120084205',
'20120084206',
'20120084207',
'20120084208',
'20120084210',
'20120084211',
'20120084230',
'20120085826',
'20120089481',
'20120089483',
'20120089506',
'20120089520',
'20120089535',
'20120089536',
'20120091199',
'20120091200',
'20120091201',
'20120094639',
'20120095851',
'20120095852',
'20120095856',
'20120095857',
'20120095867',
'20120095868',
'20120095869',
'20120095870',
'20120095871',
'20120095905',
'20120095906',
'20120095907',
'20120095914',
'20120095915',
'20120095916',
'20120095917',
'20120095918',
'20120095919',
'20120095939',
'20120096557',
'20120097739',
'20120097740',
'20120099780',
'20120101930',
'20120101937',
'20120101938',
'20120101945',
'20120101950',
'20120101960',
'20120104090',
'20120106805',
'20120108296',
'20120109762',
'20120109764',
'20120109818',
'20120109826',
'20120109829',
'20120109848',
'20120110341',
'20120110516',
'20120116880',
'20120116887',
'20120116957',
'20120116963',
'20120116965',
'20120116967',
'20120116968',
'20120116970',
'20120116973',
'20120116976',
'20120116977',
'20120118956',
'20120118959',
'20120118960',
'20120123806',
'20120123810',
'20120123824',
'20120123868',
'20120123931',
'20120123935',
'20120123937',
'20120123941',
'20120123944',
'20120123947',
'20120123966',
'20120123968',
'20120124651',
'20120124658',
'20120124659',
'20120124676',
'20120126005',
'20120126006',
'20120126007',
'20120126010',
'20120126011',
'20120126012',
'20120126013',
'20120126014',
'20120129492',
'20120129514',
'20120130783',
'20120130831',
'20120130832',
'20120130895',
'20120130896',
'20120130898',
'20120130901',
'20120130903',
'20120130904',
'20120131121',
'20120132704',
'20120132706',
'20120132712',
'20120136732',
'20120136733',
'20120136735',
'20120136763',
'20120136781',
'20120136786',
'20120136795',
'20120136796',
'20120136797',
'20120136798',
'20120137128',
'20120138675',
'20120138683',
'20120140993',
'20120143706',
'20120143707',
'20120143752',
'20120143754',
'20120143758',
'20120143767',
'20120143768',
'20120143770',
'20120143771',
'20120143772',
'20120144203',
'20120144498',
'20120150659',
'20120150673',
'20120150687',
'20120150697',
'20120150721',
'20120150722',
'20120150723',
'20120150749',
'20120150767',
'20120150768',
'20120150779',
'20120158435',
'20120158436',
'20120158541',
'20120158581',
'20120158582',
'20120158585',
'20120158591',
'20120159177',
'20120159591',
'20120159599',
'20120159612',
'20120159647',
'20120160912',
'20120166295',
'20120166334',
'20120166357',
'20120166359',
'20120166810',
'20120168505',
'20120173325',
'20120173407',
'20120173427',
'20120173429',
'20120173431',
'20120173432',
'20120173434',
'20120173457',
'20120173582',
'20120173723',
'20120178433',
'20120179493',
'20120179911',
'20120179913',
'20120180118',
'20120181333',
'20120185296',
'20120185316',
'20120185317',
'20120185338',
'20120185386',
'20120185407',
'20120185408',
'20120185410',
'20120187199',
'20120191553',
'20120191556',
'20120191569',
'20120191598',
'20120191603',
'20120191606',
'20120191607',
'20120191611',
'20120191613',
'20120191614',
'20120191624',
'20120193434',
'20120196586',
'20120197607',
'20120197669',
'20120197691',
'20120197740',
'20120197743',
'20120197761',
'20120197783',
'20120197787',
'20120197796',
'20120197797',
'20120197798',
'20120197801',
'20120197802',
'20120197803',
'20120197807',
'20120197808',
'20120197818',
'20120198548',
'20120203605',
'20120203610',
'20120203613',
'20120203646',
'20120203662',
'20120203665',
'20120203672',
'20120203673',
'20120203688',
'20120203693',
'20120203695',
'20120203696',
'20120203697',
'20120203698',
'20120203700',
'20120203702',
'20120204246',
'20120204419',
'20120205443',
'20120205450',
'20120205451',
'20120209630',
'20120209749',
'20120209769',
'20120209773',
'20120209775',
'20120209777',
'20120209789',
'20120210130',
'20120211558',
'20120215658',
'20120215691',
'20120215720',
'20120217298',
'20120221359',
'20120221446',
'20120221452',
'20120221463',
'20120221466',
'20120221468',
'20120221470',
'20120221472',
'20120221475',
'20120221859',
'20120222055',
'20120223140',
'20120226421',
'20120226582',
'20120226590',
'20120226610',
'20120226611',
'20120226613',
'20120230539',
'20120230577',
'20120231844',
'20120233004',
'20120233005',
'20120233061',
'20120233066',
'20120233665',
'20120234918',
'20120234927',
'20120235794',
'20120239375',
'20120239542',
'20120239553',
'20120239557',
'20120239559',
'20120239565',
'20120239570',
'20120239572',
'20120239574',
'20120239575',
'20120239576',
'20120239577',
'20120239579',
'20120239928',
'20120240195',
'20120240203',
'20120240227',
'20120241519',
'20120241523',
'20120244885',
'20120245967',
'20120246018',
'20120246044',
'20120246048',
'20120246071',
'20120253913',
'20120253953',
'20120253957',
'20120253978',
'20120253980',
'20120254018',
'20120254066',
'20120254067',
'20120255995',
'20120255996',
'20120258731',
'20120259666',
'20120259698',
'20120259734',
'20120259770',
'20120259778',
'20120259779',
'20120259781',
'20120259782',
'20120259783',
'20120259797',
'20120260322',
'20120261471',
'20120264405',
'20120265624',
'20120265631',
'20120265661',
'20120265679',
'20120265684',
'20120265686',
'20120265689',
'20120265690',
'20120265720',
'20120265809',
'20120266227',
'20120267432',
'20120270528',
'20120271762',
'20120271766',
'20120271771',
'20120272307',
'20120278108',
'20120278143',
'20120278155',
'20120278193',
'20120278201',
'20120278217',
'20120278241',
'20120278246',
'20120278249',
'20120278868',
'20120280035',
'20120280036',
'20120280038',
'20120280039',
'20120280048',
'20120284101',
'20120284115',
'20120284157',
'20120284194',
'20120284195',
'20120284526',
'20120284798',
'20120286928',
'20120286930',
'20120289188',
'20120290376',
'20120290449',
'20120290468',
'20120290472',
'20120290474',
'20120290478',
'20120290480',
'20120290482',
'20120290484',
'20120290609',
'20120292388',
'20120293303',
'20120296692',
'20120296768',
'20120296773',
'20120296818',
'20120296822',
'20120296824',
'20120296831',
'20120296851',
'20120297446',
'20120298757',
'20120300246',
'20120303448',
'20120303503',
'20120303505',
'20120303525',
'20120303960',
'20120304273',
'20120305645',
'20120310677',
'20120310682',
'20120310683',
'20120310684',
'20120310685',
'20120310686',
'20120310687',
'20120310795',
'20120310796',
'20120310797',
'20120310798',
'20120310806',
'20120310807',
'20120310832',
'20120310844',
'20120310845',
'20120311320',
'20120315884',
'20120316901',
'20120316944',
'20120317014',
'20120317025',
'20120317030',
'20120317035',
'20120317036',
'20120317037',
'20120317052',
'20120317628',
'20120318863',
'20120318864',
'20120321141',
'20120323709',
'20120323717',
'20120323733',
'20120323734',
'20120323762',
'20120323773',
'20120323777',
'20120323779',
'20120323786',
'20120323788',
'20120323816',
'20120324223',
'20120324242',
'20120324555',
'20120324559',
'20120328164',
'20120330729',
'20120330737',
'20120330764',
'20120330765',
'20120330779',
'20120330784',
'20120330787',
'20120330788',
'20120330824',
'20120330832',
'20120330833',
'20120330834',
'20120330835',
'20120330836',
'20120330843',
'20120330844',
'20120330845',
'20120330861',
'20120331074',
'20120331557',
'20130005253',
'20130006684',
'20130006781',
'20130006825',
'20130006848',
'20130006862',
'20130006864',
'20130006872',
'20130008947',
'20130008948',
'20130008956',
'20130013379',
'20130013433',
'20130013437',
'20130013490',
'20130013494',
'20130013503',
'20130013512',
'20130013925',
'20130014207',
'20130015241',
'20130018757',
'20130018758',
'20130018776',
'20130018779',
'20130018782',
'20130018785',
'20130018787',
'20130018791',
'20130018793',
'20130018798',
'20130018800',
'20130019287',
'20130022231',
'20130024300',
'20130024339',
'20130024354',
'20130024358',
'20130024360',
'20130024361',
'20130024366',
'20130024372',
'20130024373',
'20130024374',
'20130024375',
'20130024376',
'20130024378',
'20130024383',
'20130024386',
'20130024916',
'20130024918',
'20130024947',
'20130026232',
'20130030933',
'20130030934',
'20130030941',
'20130030995',
'20130030997',
'20130031003',
'20130031004',
'20130031005',
'20130031359',
'20130036000',
'20130036017',
'20130036027',
'20130036033',
'20130036045',
'20130036050',
'20130036051',
'20130036053',
'20130036058',
'20130036463',
'20130036480',
'20130037607',
'20130037611',
'20130041734',
'20130041807',
'20130041821',
'20130041822',
'20130041823',
'20130041848',
'20130042111',
'20130042314',
'20130043305',
'20130046645',
'20130046675',
'20130046685',
'20130046686',
'20130046690',
'20130046692',
'20130047202',
'20130048712',
'20130050088',
'20130051610',
'20130054314',
'20130054320',
'20130054337',
'20130054389',
'20130054395',
'20130054412',
'20130054417',
'20130054447',
'20130054466',
'20130054474',
'20130055362',
'20130056536',
'20130056541',
'20130060647',
'20130060670',
'20130060688',
'20130060698',
'20130060706',
'20130060716',
'20130061304',
'20130061305',
'20130062410',
'20130062420',
'20130066772',
'20130066781',
'20130066788',
'20130066804',
'20130066942',
'20130066976',
'20130067234',
'20130068835',
'20130068836',
'20130073347',
'20130073404',
'20130073448',
'20130073463',
'20130073467',
'20130073863',
'20130080276',
'20130080322',
'20130080329',
'20130080333',
'20130080351',
'20130081119',
'20130082103',
'20130083926',
'20130084797',
'20130085769',
'20130085887',
'20130085931',
'20130085935',
'20130086375',
'20130090150',
'20130091012',
'20130091028',
'20130091060',
'20130091062',
'20130091561',
'20130094751',
'20130095754',
'20130095755',
'20130095810',
'20130097034',
'20130097060',
'20130097078',
'20130097087',
'20130097429',
'20130097698',
'20130102283',
'20130103416',
'20130103544',
'20130103555',
'20130103572',
'20130103573',
'20130103612',
'20130103629',
'20130103948',
'20130104022',
'20130104213',
'20130104214',
'20130105573',
'20130109307',
'20130110658',
'20130110692',
'20130110708',
'20130110715',
'20130110717',
'20130110721',
'20130110722',
'20130110727',
'20130110728',
'20130112756',
'20130117138',
'20130117155',
'20130117184',
'20130117185',
'20130117186',
'20130117832',
'20130119147',
'20130120595',
'20130124229',
'20130124346',
'20130124364',
'20130124379',
'20130124393',
'20130124394',
'20130124396',
'20130124398',
'20130124405',
'20130124410',
'20130124411',
'20130124414',
'20130124424',
'20130124425',
'20130124855',
'20130125211',
'20130126604',
'20130126607',
'20130126608',
'20130132184',
'20130132215',
'20130132217',
'20130132219',
'20130132269',
'20130132274',
'20130132275',
'20130132281',
'20130132303',
'20130132568',
'20130133087',
'20130134214',
'20130138516',
'20130138525',
'20130138565',
'20130138566',
'20130138567',
'20130138568',
'20130138571',
'20130140360',
'20130140361',
'20130140363',
'20130144656',
'20130144734',
'20130144777',
'20130144779',
'20130144804',
'20130146659',
'20130146660',
'20130146662',
'20130148862',
'20130150101',
'20130151288',
'20130151344',
'20130151349',
'20130151358',
'20130151397',
'20130151402',
'20130151408',
'20130151410',
'20130151418',
'20130151419',
'20130159024',
'20130159118',
'20130159119',
'20130159156',
'20130159170',
'20130159186',
'20130159196',
'20130159716',
'20130160100',
'20130160144',
'20130161388',
'20130166402',
'20130166437',
'20130166454',
'20130166455',
'20130166476',
'20130167213',
'20130168450',
'20130168453',
'20130170693',
'20130171967',
'20130173237',
'20130173359',
'20130173403',
'20130173407',
'20130173450',
'20130173456',
'20130173461',
'20130173462',
'20130173743',
'20130178233',
'20130179198',
'20130179215',
'20130179216',
'20130179281',
'20130179285',
'20130179316',
'20130179335',
'20130179346',
'20130179348',
'20130179351',
'20130179352',
'20130179353',
'20130182845',
'20130182913',
'20130185124',
'20130185132',
'20130185152',
'20130185167',
'20130185189',
'20130185202',
'20130185207',
'20130185208',
'20130185559',
'20130191232',
'20130191277',
'20130191278',
'20130191279',
'20130191281',
'20130191282',
'20130191283',
'20130191284',
'20130191286',
'20130191288',
'20130191289',
'20130191789',
'20130191889',
'20130197969',
'20130197998',
'20130198018',
'20130198035',
'20130198046',
'20130198061',
'20130198066',
'20130198071',
'20130198075',
'20130198077',
'20130198078',
'20130198080',
'20130198081',
'20130198084',
'20130198086',
'20130198516',
'20130198518',
'20130198598',
'20130198834',
'20130198851',
'20130200147',
'20130200148',
'20130200149',
'20130200153',
'20130200154',
'20130200999',
'20130204645',
'20130204756',
'20130204782',
'20130204785',
'20130204788',
'20130204789',
'20130204790',
'20130204791',
'20130204792',
'20130204794',
'20130205133',
'20130205136',
'20130212003',
'20130212006',
'20130212018',
'20130212019',
'20130212026',
'20130212640',
'20130212655',
'20130212666',
'20130218601',
'20130218641',
'20130218721',
'20130218757',
'20130218769',
'20130218772',
'20130218807',
'20130219481',
'20130219506',
'20130223696',
'20130223721',
'20130226635',
'20130226717',
'20130226720',
'20130226784',
'20130226787',
'20130226789',
'20130226797',
'20130226799',
'20130226802',
'20130226803',
'20130226812',
'20130226813',
'20130226828',
'20130226829',
'20130226830',
'20130227666',
'20130227703',
'20130232040',
'20130232073',
'20130232074',
'20130232079',
'20130232083',
'20130238447',
'20130238455',
'20130238456',
'20130238482',
'20130238488',
'20130238489',
'20130238490',
'20130238491',
'20130238492',
'20130238497',
'20130238499',
'20130238503',
'20130238527',
'20130238539',
'20130239185',
'20130239187',
'20130240622',
'20130243188',
'20130244210',
'20130246086',
'20130246101',
'20130246141',
'20130246189',
'20130246200',
'20130246203',
'20130246220',
'20130246264',
'20130246268',
'20130246269',
'20130246271',
'20130246272',
'20130246280',
'20130246282',
'20130247146',
'20130248596',
'20130254070',
'20130254100',
'20130254103',
'20130254112',
'20130254117',
'20130254118',
'20130254135',
'20130254862',
'20130256421',
'20130260734',
'20130262237',
'20130262298',
'20130262302',
'20130262303',
'20130262305',
'20130262311',
'20130262314',
'20130262315',
'20130262316',
'20130262317',
'20130262342',
'20130264384',
'20130265136',
'20130268324',
'20130268337',
'20130268339',
'20130268378',
'20130268435',
'20130268437',
'20130268439',
'20130268441',
'20130268443',
'20130269003',
'20130275199',
'20130275293',
'20130275301',
'20130275303',
'20130275307',
'20130275310',
'20130275331',
'20130275334',
'20130275355',
'20130276140',
'20130278622',
'20130282502',
'20130282562',
'20130282567',
'20130282576',
'20130282577',
'20130282580',
'20130282588',
'20130282590',
'20130282591',
'20130287265',
'20130287284',
'20130290036',
'20130290119',
'20130290136',
'20130290165',
'20130290176',
'20130290182',
'20130290186',
'20130290192',
'20130290217',
'20130294661',
'20130297353',
'20130297414',
'20130297418',
'20130297432',
'20130297489',
'20130297493',
'20130297494',
'20130297497',
'20130297505',
'20130297512',
'20130297514',
'20130298208',
'20130299575',
'20130299585',
'20130303084',
'20130303113',
'20130304515',
'20130304519',
'20130304637',
'20130304638',
'20130304648',
'20130304649',
'20130304651',
'20130304671',
'20130305035',
'20130305335',
'20130307670',
'20130308778',
'20130311313',
'20130311330',
'20130311344',
'20130311355',
'20130311358',
'20130311372',
'20130311382',
'20130311383',
'20130311398',
'20130312073',
'20130317665',
'20130317736',
'20130317862',
'20130317896',
'20130317923',
'20130317924',
'20130317927',
'20130317984',
'20130317985',
'20130317989',
'20130317991',
'20130318592',
'20130318619',
'20130318622',
'20130320084',
'20130320097',
'20130325517',
'20130325661',
'20130325709',
'20130325718',
'20130325724',
'20130325748',
'20130325751',
'20130327825',
'20130328801',
'20130329888',
'20130332232',
'20130332256',
'20130332293',
'20130332338',
'20130332343',
'20130332353',
'20130332360',
'20130332365',
'20130332366',
'20130332367',
'20130332394',
'20130333006',
'20130333008',
'20130333011',
'20130334308',
'20130339188',
'20130339218',
'20130339232',
'20130339234',
'20130339237',
'20130339242',
'20130339245',
'20130339247',
'20130339249',
'20130339747',
'20130340091',
'20130346115',
'20130346222',
'20130346243',
'20130346245',
'20130346246',
'20130346264',
'20130346287',
'20130346310',
'20130346311',
'20130346312',
'20130346313',
'20130346314',
'20130346325',
'20130346343',
'20130347075',
'20140001257',
'20140001263',
'20140001264',
'20140002238',
'20140006048',
'20140006149',
'20140006183',
'20140006184',
'20140006190',
'20140006196',
'20140006198',
'20140006255',
'20140006259',
'20140006272',
'20140006274',
'20140006275',
'20140006276',
'20140006277',
'20140006280',
'20140006281',
'20140006284',
'20140006289',
'20140012633',
'20140012683',
'20140012689',
'20140012734',
'20140012754',
'20140012757',
'20140012768',
'20140012778',
'20140014718',
'20140019331',
'20140019340',
'20140019344',
'20140019358',
'20140019363',
'20140019364',
'20140019367',
'20140019380',
'20140020068',
'20140025472',
'20140025515',
'20140025516',
'20140025517',
'20140025520',
'20140025575',
'20140025580',
'20140025581',
'20140025585',
'20140025958',
'20140031011',
'20140032312',
'20140032345',
'20140032381',
'20140032384',
'20140032406',
'20140032409',
'20140032414',
'20140032419',
'20140034728',
'20140039938',
'20140040044',
'20140040052',
'20140040113',
'20140040123',
'20140040139',
'20140040144',
'20140040147',
'20140040155',
'20140041000',
'20140041018',
'20140044303',
'20140046701',
'20140046746',
'20140046785',
'20140046819',
'20140046835',
'20140046837',
'20140046844',
'20140046845',
'20140046848',
'20140046851',
'20140046853',
'20140047233',
'20140048597',
'20140052532',
'20140052553',
'20140052564',
'20140052604',
'20140052620',
'20140052630',
'20140052632',
'20140052633',
'20140052636',
'20140052637',
'20140052642',
'20140054369',
'20140058761',
'20140058854',
'20140058858',
'20140058862',
'20140058893',
'20140058914',
'20140058926',
'20140058937',
'20140058939',
'20140058944',
'20140058948',
'20140059657',
'20140061298',
'20140061302',
'20140064566',
'20140064573',
'20140066016',
'20140067434',
'20140067566',
'20140067572',
'20140067575',
'20140067634',
'20140067670',
'20140067672',
'20140067673',
'20140067674',
'20140067675',
'20140067683',
'20140067712',
'20140067713',
'20140067714',
'20140067715',
'20140067716',
'20140068722',
'20140068742',
'20140070000',
'20140070001',
'20140072185',
'20140074569',
'20140074605',
'20140074635',
'20140074637',
'20140074658',
'20140074689',
'20140074691',
'20140074696',
'20140074697',
'20140074698',
'20140074715',
'20140074717',
'20140074722',
'20140074723',
'20140074756',
'20140076964',
'20140080100',
'20140080470',
'20140081745',
'20140081750',
'20140081827',
'20140081832',
'20140081835',
'20140081844',
'20140081849',
'20140081853',
'20140081854',
'20140081857',
'20140081862',
'20140081866',
'20140081874',
'20140084059',
'20140087335',
'20140089041',
'20140089113',
'20140089120',
'20140089153',
'20140089169',
'20140089184',
'20140089185',
'20140089188',
'20140089191',
'20140089194',
'20140089195',
'20140089197',
'20140089203',
'20140089204',
'20140089205',
'20140090021',
'20140090039',
'20140090045',
'20140095251',
'20140095331',
'20140095382',
'20140095383',
'20140096216',
'20140097241',
'20140097242',
'20140097249',
'20140099001',
'20140099886',
'20140099915',
'20140100892',
'20140100920',
'20140100921',
'20140100931',
'20140100973',
'20140101030',
'20140101035',
'20140101036',
'20140101042',
'20140101046',
'20140101055',
'20140106717',
'20140108058',
'20140108108',
'20140108172',
'20140108173',
'20140108240',
'20140108241',
'20140108247',
'20140108251',
'20140108257',
'20140108258',
'20140108260',
'20140108261',
'20140108298',
'20140110477',
'20140114699',
'20140114703',
'20140114852',
'20140114855',
'20140114857',
'20140114861',
'20140114878',
'20140114881',
'20140115492',
'20140115717',
'20140117075',
'20140120986',
'20140122171',
'20140122265',
'20140122267',
'20140122307',
'20140122326',
'20140122328',
'20140122331',
'20140122334',
'20140122336',
'20140122339',
'20140122563',
'20140122688',
'20140123224',
'20140123256',
'20140129402',
'20140129422',
'20140129428',
'20140129430',
'20140129431',
'20140129437',
'20140129438',
'20140129448',
'20140129450',
'20140129451',
'20140129482',
'20140130035',
'20140131432',
'20140131442',
'20140136329',
'20140136352',
'20140136354',
'20140136382',
'20140136383',
'20140136397',
'20140136402',
'20140136404',
'20140136419',
'20140136420',
'20140137199',
'20140137235',
'20140138435',
'20140138436',
'20140139731',
'20140143037',
'20140143075',
'20140143104',
'20140143134',
'20140143145',
'20140143146',
'20140143147',
'20140143152',
'20140143173',
'20140143177',
'20140144983',
'20140149179',
'20140149285',
'20140149286',
'20140149291',
'20140149293',
'20140149295',
'20140149317',
'20140149518',
'20140150100',
'20140156424',
'20140156448',
'20140156501',
'20140156507',
'20140156512',
'20140156525',
'20140156527',
'20140156534',
'20140156560',
'20140156568',
'20140158761',
'20140158767',
'20140158768',
'20140158773',
'20140162598',
'20140164024',
'20140164053',
'20140164083',
'20140164092',
'20140164154',
'20140164156',
'20140164157',
'20140164228',
'20140164229',
'20140164241',
'20140164243',
'20140164244',
'20140164250',
'20140164251',
'20140164253',
'20140164254',
'20140164268',
'20140166749',
'20140171024',
'20140172465',
'20140172507',
'20140172551',
'20140172577',
'20140172597',
'20140172598',
'20140172683',
'20140172686',
'20140172687',
'20140172690',
'20140172693',
'20140172694',
'20140172701',
'20140172710',
'20140172711',
'20140172724',
'20140172752',
'20140173284',
'20140175178',
'20140180557',
'20140180714',
'20140180727',
'20140180850',
'20140180852',
'20140180906',
'20140180918',
'20140180926',
'20140181908',
'20140183258',
'20140183259',
'20140187197',
'20140187205',
'20140188639',
'20140188640',
'20140188697',
'20140188702',
'20140188707',
'20140188714',
'20140188717',
'20140188718',
'20140188721',
'20140188723',
'20140188727',
'20140188728',
'20140188730',
'20140188735',
'20140188759',
'20140189359',
'20140189794',
'20140189835',
'20140191028',
'20140195347',
'20140195361',
'20140195362',
'20140195416',
'20140195429',
'20140195434',
'20140195441',
'20140195447',
'20140196118',
'20140201077',
'20140201080',
'20140201537',
'20140203088',
'20140207575',
'20140207668',
'20140207672',
'20140207675',
'20140207680',
'20140207684',
'20140208419',
'20140209672',
'20140209673',
'20140210589',
'20140214642',
'20140214643',
'20140214648',
'20140214649',
'20140214654',
'20140214655',
'20140214662',
'20140214668',
'20140214669',
'20140214670',
'20140214673',
'20140214674',
'20140214677',
'20140214679',
'20140215596',
'20140217169',
'20140222505',
'20140222595',
'20140222599',
'20140222631',
'20140222635',
'20140222655',
'20140222664',
'20140222678',
'20140222681',
'20140222682',
'20140222689',
'20140222691',
'20140222714',
'20140222716',
'20140223171',
'20140223510',
'20140223578',
'20140223584',
'20140224883',
'20140229259',
'20140229276',
'20140229304',
'20140229370',
'20140229377',
'20140229379',
'20140229382',
'20140229408',
'20140231527',
'20140235198',
'20140235202',
'20140236813',
'20140236819',
'20140236821',
'20140236829',
'20140237240',
'20140237242',
'20140239065',
'20140241523',
'20140244315',
'20140244462',
'20140244483',
'20140244494',
'20140244495',
'20140244496',
'20140244503',
'20140244506',
'20140244511',
'20140244514',
'20140244543',
'20140245380',
'20140246490',
'20140249901',
'20140249917',
'20140249922',
'20140249939',
'20140249940',
'20140249946',
'20140249948',
'20140249985',
'20140250005',
'20140250006',
'20140250007',
'20140250009',
'20140250010',
'20140250011',
'20140250013',
'20140250016',
'20140250018',
'20140250026',
'20140256251',
'20140257592',
'20140257863',
'20140257869',
'20140257870',
'20140257874',
'20140258009',
'20140258089',
'20140258096',
'20140258109',
'20140258110',
'20140258120',
'20140258123',
'20140258125',
'20140258133',
'20140258136',
'20140258175',
'20140258707',
'20140263608',
'20140263618',
'20140263621',
'20140263623',
'20140263625',
'20140263626',
'20140263627',
'20140263646',
'20140266596',
'20140266597',
'20140266598',
'20140266599',
'20140266601',
'20140266602',
'20140266603',
'20140266605',
'20140266606',
'20140270173',
'20140270174',
'20140270175',
'20140270461',
'20140270462',
'20140270463',
'20140270464',
'20140273959',
'20140273960',
'20140273961',
'20140273996',
'20140278587',
'20140278609',
'20140278736',
'20140278740',
'20140278774',
'20140279094',
'20140279101',
'20140279106',
'20140279185',
'20140279228',
'20140279309',
'20140279327',
'20140279387',
'20140279389',
'20140279398',
'20140279401',
'20140279402',
'20140279403',
'20140279411',
'20140279416',
'20140279417',
'20140279436',
'20140279446',
'20140279447',
'20140279451',
'20140279457',
'20140279465',
'20140279466',
'20140279472',
'20140279476',
'20140279477',
'20140279479',
'20140279480',
'20140279481',
'20140279482',
'20140279483',
'20140279490',
'20140279491',
'20140279493',
'20140279494',
'20140279497',
'20140279498',
'20140279499',
'20140279500',
'20140279503',
'20140279515',
'20140279518',
'20140279529',
'20140279530',
'20140279532',
'20140279542',
'20140279545',
'20140279546',
'20140279554',
'20140279556',
'20140279560',
'20140279565',
'20140279693',
'20140279701',
'20140281565',
'20140281566',
'20140282877',
'20140282973',
'20140282987',
'20140282993',
'20140288989',
'20140289061',
'20140289103',
'20140289113',
'20140289116',
'20140289119',
'20140289120',
'20140289124',
'20140289125',
'20140289131',
'20140289132',
'20140289133',
'20140289790',
'20140289833',
'20140291391',
'20140291406',
'20140297438',
'20140297518',
'20140297527',
'20140297533',
'20140298011',
'20140298412',
'20140299664',
'20140300739',
'20140302824',
'20140303833',
'20140304055',
'20140304073',
'20140304082',
'20140304094',
'20140304095',
'20140304132',
'20140304152',
'20140304157',
'20140304158',
'20140304159',
'20140304160',
'20140304161',
'20140304163',
'20140304169',
'20140304192',
'20140304193',
'20140304828',
'20140308934',
'20140310027',
'20140310076',
'20140310113',
'20140310137',
'20140310153',
'20140310157',
'20140310160',
'20140310161',
'20140310171',
'20140310172',
'20140310174',
'20140310176',
'20140310182',
'20140310184',
'20140310764',
'20140312118',
'20140316852',
'20140316855',
'20140316984',
'20140316986',
'20140317022',
'20140317414',
'20140317715',
'20140317734',
'20140319212',
'20140319213',
'20140324519',
'20140324538',
'20140324560',
'20140324574',
'20140324606',
'20140324635',
'20140324677',
'20140324683',
'20140324692',
'20140324697',
'20140325589',
'20140325606',
'20140325678',
'20140330626',
'20140330656',
'20140330675',
'20140330697',
'20140330706',
'20140330711',
'20140330722',
'20140331058',
'20140332588',
'20140333414',
'20140337138',
'20140337205',
'20140337215',
'20140337216',
'20140337218',
'20140337221',
'20140337224',
'20140337227',
'20140337232',
'20140337233',
'20140337234',
'20140337920',
'20140337942',
'20140337957',
'20140339304',
'20140339315',
'20140343972',
'20140344042',
'20140344069',
'20140344081',
'20140344082',
'20140344085',
'20140344140',
'20140344150',
'20140344151',
'20140344153',
'20140344154',
'20140344155',
'20140344164',
'20140344166',
'20140344580',
'20140349575',
'20140349692',
'20140351126',
'20140351139',
'20140351140',
'20140351144',
'20140351145',
'20140351146',
'20140351147',
'20140351166',
'20140351168',
'20140351899',
'20140351910',
'20140358326',
'20140358634',
'20140358704',
'20140358706',
'20140358707',
'20140358708',
'20140358727',
'20140358769',
'20140358776',
'20140358778',
'20140358781',
'20140358783',
'20140358785',
'20140358787',
'20140358788',
'20140358794',
'20140358795',
'20140358796',
'20140358799',
'20140365356',
'20140365366',
'20140365371',
'20140365373',
'20140365374',
'20140365375',
'20140365400',
'20140365776',
'20140366151',
'20140367467',
'20140367474',
'20140368336',
'20140372128',
'20140372284',
'20140372294',
'20140372295',
'20140372301',
'20140372305',
'20140372308',
'20140372318',
'20140372320',
'20140372322',
'20140372323',
'20140373093',
'20140373114',
'20140373170',
'20140379470',
'20140379563',
'20140379581',
'20140379582',
'20140379583',
'20140379584',
'20140380444',
'20140380497',
'20150001289',
'20150004934',
'20150006207',
'20150006271',
'20150006305',
'20150006378',
'20150006384',
'20150006385',
'20150006388',
'20150006390',
'20150006406',
'20150006407',
'20150007301',
'20150012334',
'20150012346',
'20150012382',
'20150012388',
'20150012399',
'20150012420',
'20150012422',
'20150012426',
'20150012427',
'20150012429',
'20150012434',
'20150012435',
'20150012436',
'20150012440',
'20150012442',
'20150012444',
'20150012489',
'20150012979',
'20150014413',
'20150019293',
'20150019320',
'20150019375',
'20150019405',
'20150019418',
'20150019424',
'20150019432',
'20150019439',
'20150019441',
'20150019442',
'20150019443',
'20150019449',
'20150021404',
'20150023600',
'20150025946',
'20150025987',
'20150025988',
'20150026025',
'20150026026',
'20150026027',
'20150026035',
'20150026039',
'20150026045',
'20150026050',
'20150026053',
'20150026054',
'20150026055',
'20150026056',
'20150026057',
'20150026058',
'20150026060',
'20150026061',
'20150026063',
'20150026064',
'20150026065',
'20150026066',
'20150026067',
'20150026070',
'20150026072',
'20150026098',
'20150026771',
'20150026776',
'20150031393',
'20150032481',
'20150032524',
'20150032527',
'20150032543',
'20150032565',
'20150032578',
'20150032585',
'20150032589',
'20150032598',
'20150032614',
'20150032621',
'20150032623',
'20150032625',
'20150032626',
'20150032627',
'20150032635',
'20150033286',
'20150033289',
'20150039214',
'20150039348',
'20150039454',
'20150039455',
'20150039494',
'20150039503',
'20150039506',
'20150039512',
'20150039533',
'20150041534',
'20150041548',
'20150044987',
'20150044992',
'20150046197',
'20150046216',
'20150046220',
'20150046224',
'20150046240',
'20150046257',
'20150046282',
'20150046302',
'20150046317',
'20150046320',
'20150046328',
'20150046331',
'20150046332',
'20150046336',
'20150046338',
'20150046340',
'20150047000',
'20150047007',
'20150050987',
'20150051947',
'20150051953',
'20150052005',
'20150052036',
'20150052053',
'20150052061',
'20150052062',
'20150052064',
'20150052065',
'20150052587',
'20150056957',
'20150058088',
'20150058145',
'20150058211',
'20150058216',
'20150058225',
'20150058227',
'20150058260',
'20150060540',
'20150060542',
'20150066577',
'20150066651',
'20150066670',
'20150066714',
'20150066762',
'20150066764',
'20150066768',
'20150066771',
'20150066772',
'20150066775',
'20150066779',
'20150066808',
'20150066809',
'20150066811',
'20150067786',
'20150069131',
'20150072744',
'20150073869',
'20150073907',
'20150073953',
'20150073956',
'20150073957',
'20150073981',
'20150073984',
'20150073985',
'20150073988',
'20150073996',
'20150077228',
'20150079326',
'20150081355',
'20150081378',
'20150081405',
'20150081461',
'20150081492',
'20150081523',
'20150081524',
'20150081540',
'20150081541',
'20150081542',
'20150081543',
'20150081544',
'20150081545',
'20150081546',
'20150081552',
'20150081554',
'20150081556',
'20150081559',
'20150081566',
'20150082025',
'20150082042',
'20150084736',
'20150088564',
'20150088595',
'20150088673',
'20150088674',
'20150088677',
'20150088738',
'20150088742',
'20150088745',
'20150088746',
'20150088749',
'20150088750',
'20150088751',
'20150088754',
'20150088756',
'20150088757',
'20150088758',
'20150088759',
'20150088783',
'20150089585',
'20150089613',
'20150093048',
'20150095174',
'20150095175',
'20150095219',
'20150095222',
'20150095224',
'20150095226',
'20150095227',
'20150095234',
'20150095235',
'20150095236',
'20150095238',
'20150095239',
'20150095240',
'20150095265',
'20150095986',
'20150095990',
'20150097028',
'20150100478',
'20150100483',
'20150100487',
'20150100488',
'20150100491',
'20150100499',
'20150101059',
'20150106186',
'20150106239',
'20150106259',
'20150106269',
'20150106275',
'20150108227',
'20150112802',
'20150112866',
'20150112869',
'20150112870',
'20150112871',
'20150112872',
'20150113616',
'20150113625',
'20150118958',
'20150119006',
'20150120428',
'20150120538',
'20150120539',
'20150120544',
'20150120549',
'20150120556',
'20150120559',
'20150120560',
'20150120571',
'20150120572',
'20150121066',
'20150121464',
'20150121475',
'20150121501',
'20150126109',
'20150127475',
'20150127523',
'20150127524',
'20150127529',
'20150127535',
'20150127536',
'20150127547',
'20150127549',
'20150127550',
'20150127553',
'20150127581',
'20150134507',
'20150134508',
'20150134512',
'20150134514',
'20150134519',
'20150134537',
'20150134539',
'20150134542',
'20150136848',
'20150142446',
'20150142514',
'20150142541',
'20150142542',
'20150142545',
'20150142592',
'20150142593',
'20150142596',
'20150142604',
'20150142638',
'20150142640',
'20150142642',
'20150142643',
'20150142644',
'20150142651',
'20150142654',
'20150142655',
'20150142665',
'20150142666',
'20150142667',
'20150142669',
'20150142670',
'20150142671',
'20150142672',
'20150142673',
'20150142713',
'20150143106',
'20150143116',
'20150143129',
'20150145706',
'20150149219',
'20150149291',
'20150149309',
'20150149311',
'20150149343',
'20150149344',
'20150149350',
'20150149364',
'20150149365',
'20150149992',
'20150154581',
'20150154586',
'20150154594',
'20150154595',
'20150154596',
'20150154597',
'20150154600',
'20150154712',
'20150156176',
'20150161417',
'20150161419',
'20150161538',
'20150161576',
'20150161586',
'20150161588',
'20150161593',
'20150161597',
'20150161602',
'20150161608',
'20150161610',
'20150161611',
'20150161612',
'20150164192',
'20150166218',
'20150169312',
'20150169839',
'20150170112',
'20150170118',
'20150170129',
'20150170130',
'20150170132',
'20150170133',
'20150170135',
'20150170136',
'20150170138',
'20150170143',
'20150170145',
'20150170149',
'20150170166',
'20150170177',
'20150170236',
'20150170289',
'20150170290',
'20150172062',
'20150178515',
'20150178702',
'20150178704',
'20150178715',
'20150178719',
'20150178724',
'20150178732',
'20150178733',
'20150178749',
'20150178825',
'20150178841',
'20150178846',
'20150178862',
'20150180836',
'20150186663',
'20150186864',
'20150186867',
'20150186868',
'20150186871',
'20150186872',
'20150186876',
'20150186877',
'20150186881',
'20150186887',
'20150186889',
'20150186891',
'20150186901',
'20150186909',
'20150186983',
'20150187017',
'20150187019',
'20150187021',
'20150188891',
'20150189067',
'20150193678',
'20150193748',
'20150193763',
'20150193764',
'20150193767',
'20150193769',
'20150193773',
'20150193777',
'20150193781',
'20150193885',
'20150195707',
'20150198983',
'20150199604',
'20150199670',
'20150199674',
'20150199678',
'20150199679',
'20150199682',
'20150199684',
'20150199686',
'20150199689',
'20150200774',
'20150200934',
'20150201029',
'20150205699',
'20150205700',
'20150205701',
'20150205702',
'20150205703',
'20150205708',
'20150205712',
'20150205713',
'20150206106',
'20150206123',
'20150206130',
'20150206137',
'20150206141',
'20150206142',
'20150206144',
'20150206147',
'20150206148',
'20150206149',
'20150206150',
'20150206245',
'20150206416',
'20150207795',
'20150213350',
'20150213351',
'20150213435',
'20150213436',
'20150213437',
'20150213443',
'20150213447',
'20150213560',
'20150220892',
'20150220900',
'20150220905',
'20150220919',
'20150220920',
'20150220922',
'20150220923',
'20150220928',
'20150220930',
'20150220932',
'20150220960',
'20150221036',
'20150221149',
'20150222729',
'20150227727',
'20150227728',
'20150227897',
'20150227898',
'20150227903',
'20150227906',
'20150227912',
'20150227913',
'20150227920',
'20150227922',
'20150227923',
'20150227924',
'20150227925',
'20150227926',
'20150227927',
'20150227929',
'20150227932',
'20150227934',
'20150227935',
'20150227936',
'20150227937',
'20150228014',
'20150228015',
'20150228026',
'20150229622',
'20150229623',
'20150230045',
'20150235195',
'20150235201',
'20150235211',
'20150235214',
'20150235217',
'20150235222',
'20150235223',
'20150235225',
'20150235241',
'20150235290',
'20150235321',
'20150237028',
'20150242601',
'20150242663',
'20150242789',
'20150242823',
'20150242837',
'20150242838',
'20150242839',
'20150242840',
'20150242843',
'20150242844',
'20150242848',
'20150242853',
'20150244705',
'20150248655',
'20150248663',
'20150248664',
'20150248665',
'20150248666',
'20150248668',
'20150248670',
'20150254629',
'20150254634',
'20150254635',
'20150254639',
'20150254640',
'20150254646',
'20150254647',
'20150254648',
'20150254649',
'20150254650',
'20150254653',
'20150254655',
'20150254656',
'20150254657',
'20150254658',
'20150254660',
'20150254662',
'20150254663',
'20150254664',
'20150254665',
'20150254668',
'20150254670',
'20150254700',
'20150254746',
'20150254769',
'20150256518',
'20150257004',
'20150261948',
'20150262137',
'20150262138',
'20150262139',
'20150262140',
'20150262141',
'20150262161',
'20150262164',
'20150262165',
'20150262168',
'20150262171',
'20150262172',
'20150262173',
'20150262176',
'20150262184',
'20150262185',
'20150269371',
'20150269538',
'20150269555',
'20150269562',
'20150269566',
'20150269571',
'20150269573',
'20150269577',
'20150269578',
'20150269579',
'20150269580',
'20150269582',
'20150269662',
'20150269669',
'20150269701',
'20150270873',
'20150271149',
'20150277964',
'20150278492',
'20150278776',
'20150278797',
'20150278799',
'20150278805',
'20150278808',
'20150278809',
'20150278814',
'20150278815',
'20150278820',
'20150278843',
'20150286694',
'20150286998',
'20150287015',
'20150287016',
'20150287020',
'20150287028',
'20150287030',
'20150287031',
'20150287033',
'20150287035',
'20150287036',
'20150287037',
'20150287255',
'20150287289',
'20150288464',
'20150288521',
'20150288687',
'20150288799',
'20150294095',
'20150294208',
'20150294289',
'20150294290',
'20150294299',
'20150294303',
'20150294304',
'20150294305',
'20150294306',
'20150294307',
'20150294308',
'20150294313',
'20150294422',
'20150294437',
'20150302225',
'20150302376',
'20150302381',
'20150302390',
'20150302393',
'20150302394',
'20150302398',
'20150302401',
'20150302402',
'20150302404',
'20150302411',
'20150302413',
'20150302571',
'20150304291',
'20150310404',
'20150310419',
'20150310420',
'20150310422',
'20150310423',
'20150310424',
'20150310425',
'20150310428',
'20150310431',
'20150310436',
'20150310439',
'20150310444',
'20150310537',
'20150312248',
'20150317468',
'20150317528',
'20150317613',
'20150317625',
'20150317626',
'20150317627',
'20150317630',
'20150317631',
'20150317633',
'20150317634',
'20150317638',
'20150317726',
'20150317745',
'20150317748',
'20150319158',
'20150319161',
'20150324715',
'20150324736',
'20150324787',
'20150324792',
'20150324793',
'20150324795',
'20150324816',
'20150324924',
'20150324927',
'20150325057',
'20150326550',
'20150327071',
'20150332038',
'20150332222',
'20150332253',
'20150332254',
'20150332260',
'20150332262',
'20150332264',
'20150332266',
'20150332269',
'20150332276',
'20150332395',
'20150332407',
'20150334098',
'20150334099',
'20150339637',
'20150339641',
'20150339656',
'20150339659',
'20150339663',
'20150339664',
'20150339666',
'20150339667',
'20150339668',
'20150339671',
'20150339673',
'20150339674',
'20150339696',
'20150347734',
'20150347891',
'20150347992',
'20150347999',
'20150348000',
'20150348001',
'20150348002',
'20150348007',
'20150348008',
'20150348009',
'20150348014',
'20150348017',
'20150348022',
'20150348023',
'20150348024',
'20150348025',
'20150348026',
'20150348029',
'20150348033',
'20150348035',
'20150348037',
'20150348038',
'20150348044',
'20150348045',
'20150348197',
'20150348198',
'20150348199',
'20150355889',
'20150356534',
'20150356545',
'20150356560',
'20150356562',
'20150356566',
'20150356575',
'20150356686',
'20150363581',
'20150363684',
'20150363752',
'20150363754',
'20150363762',
'20150363769',
'20150363770',
'20150363772',
'20150363773',
'20150363777',
'20150363778',
'20150363780',
'20150363782',
'20150363783',
'20150363785',
'20150363786',
'20150363876',
'20150363886',
'20150365283',
'20150365404',
'20150371215',
'20150371220',
'20150371221',
'20150371224',
'20150371230',
'20150371232',
'20150371248',
'20150371339',
'20150372811',
'20150379505',
'20150379510',
'20150379515',
'20150379516',
'20150379615',
'20150379619',
'20160005022',
'20160005024',
'20160005027',
'20160005029',
'20160005034',
'20160005038',
'20160005039',
'20160005040',
'20160005061',
'20160005130',
'20160005131',
'20160005132',
'20160006718',
'20160012216',
'20160012403',
'20160012405',
'20160012407',
'20160012411',
'20160012427',
'20160012433',
'20160012434',
'20160012435',
'20160012436',
'20160012437',
'20160012438',
'20160012439',
'20160012440',
'20160012445',
'20160012447',
'20160012500',
'20160012524',
'20160012545',
'20160014251',
'20160014623',
'20160019450',
'20160019512',
'20160019528',
'20160019539',
'20160019543',
'20160019545',
'20160019547',
'20160019548',
'20160019614',
'20160021537',
'20160026726',
'20160026779',
'20160026782',
'20160026831',
'20160026909',
'20160026980',
'20160026990',
'20160027000',
'20160027003',
'20160027005',
'20160027006',
'20160027009',
'20160027010',
'20160027012',
'20160027015',
'20160027102',
'20160028550',
'20160028715',
'20160034680',
'20160034682',
'20160034819',
'20160034864',
'20160034866',
'20160034877',
'20160034880',
'20160034896',
'20160034897',
'20160034899',
'20160034900',
'20160034925',
'20160034971',
'20160034990',
'20160034992',
'20160035023',
'20160036790',
'20160042167',
'20160042263',
'20160042338',
'20160042341',
'20160042346',
'20160042357',
'20160042417',
'20160042420',
'20160044022',
'20160044048',
'20160048822',
'20160048823',
'20160048826',
'20160048829',
'20160048830',
'20160048832',
'20160048834',
'20160048835',
'20160048858',
'20160050199',
'20160055405',
'20160055478',
'20160055480',
'20160055483',
'20160055485',
'20160055583',
'20160055589',
'20160057129',
'20160057144',
'20160057248',
'20160057619',
'20160063227',
'20160063471',
'20160063477',
'20160063478',
'20160063480',
'20160063482',
'20160063490',
'20160063491',
'20160063493',
'20160063496',
'20160063503',
'20160063632',
'20160063634',
'20160065370',
'20160065544',
'20160065570',
'20160070940',
'20160070941',
'20160071071',
'20160071072',
'20160071076',
'20160071085',
'20160071087',
'20160071094',
'20160071096',
'20160071097',
'20160071099',
'20160071101',
'20160071104',
'20160071105',
'20160071106',
'20160071109',
'20160071110',
'20160071115',
'20160071205',
'20160071210',
'20160072800',
'20160072845',
'20160073261',
'20160078416',
'20160078417',
'20160078424',
'20160078425',
'20160078430',
'20160078434',
'20160078446',
'20160078448',
'20160078460',
'20160078540',
'20160078556',
'20160080151',
'20160080152',
'20160080156',
'20160080326',
'20160080899',
'20160080937',
'20160080944',
'20160085954',
'20160086073',
'20160086156',
'20160086159',
'20160086161',
'20160086164',
'20160086170',
'20160086171',
'20160086172',
'20160086182',
'20160086183',
'20160086217',
'20160087952',
'20160088449',
'20160092667',
'20160092696',
'20160092859',
'20160092860',
'20160092861',
'20160092862',
'20160092863',
'20160092865',
'20160092866',
'20160092869',
'20160092870',
'20160092872',
'20160092875',
'20160092876',
'20160092880',
'20160094991',
'20160098530',
'20160098693',
'20160098696',
'20160098698',
'20160098699',
'20160098705',
'20160098706',
'20160098708',
'20160098710',
'20160098711',
'20160098712',
'20160098715',
'20160098716',
'20160098719',
'20160098721',
'20160098726',
'20160098918',
'20160099923',
'20160100314',
'20160104132',
'20160104133',
'20160104136',
'20160104140',
'20160104147',
'20160104150',
'20160104153',
'20160104154',
'20160104158',
'20160104159',
'20160104160',
'20160104161',
'20160104163',
'20160104244',
'20160104251',
'20160105421',
'20160105425',
'20160109954',
'20160110529',
'20160110672',
'20160110693',
'20160110694',
'20160110695',
'20160110696',
'20160110697',
'20160110705',
'20160110707',
'20160110709',
'20160110713',
'20160110715',
'20160110716',
'20160110718',
'20160110719',
'20160110753',
'20160112872',
'20160114807',
'20160117647',
'20160117648',
'20160117651',
'20160117665',
'20160117666',
'20160117670',
'20160117677',
'20160117688',
'20160117715',
'20160117771',
'20160117772',
'20160117773',
'20160117871',
'20160117895',
'20160119296',
'20160119364',
'20160125040',
'20160125181',
'20160125193',
'20160125368',
'20160125369',
'20160125370',
'20160125371',
'20160125376',
'20160125377',
'20160125386',
'20160125394',
'20160125400',
'20160125404',
'20160125405',
'20160125406',
'20160125412',
'20160125414',
'20160125415',
'20160125416',
'20160125522',
'20160125538',
'20160125677',
'20160127904',
'20160129914',
'20160132701',
'20160132703',
'20160132848',
'20160132851',
'20160132857',
'20160132862',
'20160132863',
'20160132864',
'20160132869',
'20160132871',
'20160132877',
'20160132878',
'20160132879',
'20160132880',
'20160132882',
'20160132884',
'20160132888',
'20160132908',
'20160135020',
'20160140333',
'20160140525',
'20160140535',
'20160140536',
'20160140537',
'20160140542',
'20160140545',
'20160140547',
'20160140558',
'20160140598',
'20160140653',
'20160142397',
'20160148023',
'20160148092',
'20160148188',
'20160148190',
'20160148191',
'20160148193',
'20160148198',
'20160148201',
'20160148212',
'20160148258',
'20160149873',
'20160155102',
'20160155106',
'20160155109',
'20160155112',
'20160155114',
'20160155115',
'20160155116',
'20160155120',
'20160155121',
'20160155122',
'20160155126',
'20160155136',
'20160155160',
'20160155191',
'20160155193',
'20160155200',
'20160156608',
'20160156609',
'20160162766',
'20160162854',
'20160162857',
'20160162861',
'20160162866',
'20160162867',
'20160162872',
'20160162873',
'20160162875',
'20160162883',
'20160162887',
'20160162888',
'20160162895',
'20160162896',
'20160162902',
'20160162903',
'20160162963',
'20160164857',
'20160164858',
'20160164921',
'20160171209',
'20160171306',
'20160171461',
'20160171476',
'20160171477',
'20160171479',
'20160171480',
'20160171481',
'20160171482',
'20160171487',
'20160171493',
'20160171495',
'20160171498',
'20160171499',
'20160171500',
'20160173483',
'20160180305',
'20160180306',
'20160180307',
'20160180313',
'20160180316',
'20160180318',
'20160180319',
'20160180320',
'20160180321',
'20160180323',
'20160180324',
'20160180326',
'20160180328',
'20160180330',
'20160180331',
'20160180338',
'20160180340',
'20160180342',
'20160180452',
'20160183032',
'20160188849',
'20160188855',
'20160188915',
'20160189123',
'20160189127',
'20160189130',
'20160189134',
'20160189135',
'20160189136',
'20160189137',
'20160189138',
'20160189141',
'20160189143',
'20160189145',
'20160189151',
'20160189154',
'20160189157',
'20160189158',
'20160189161',
'20160189299',
'20160189302',
'20160191197',
'20160192123',
'20160196391',
'20160196539',
'20160196540',
'20160196544',
'20160196558',
'20160196608',
'20160196613',
'20160197725',
'20160197727',
'20160197915',
'20160203315',
'20160203364',
'20160203448',
'20160203466',
'20160203467',
'20160203469',
'20160203479',
'20160203480',
'20160203485',
'20160203486',
'20160203491',
'20160203559',
'20160203575',
'20160203667',
'20160205058',
'20160205112',
'20160210606',
'20160210613',
'20160210620',
'20160210628',
'20160210633',
'20160210634',
'20160212126',
'20160217359',
'20160217448',
'20160217452',
'20160217455',
'20160217456',
'20160217458',
'20160217462',
'20160217464',
'20160217465',
'20160217467',
'20160217470',
'20160217471',
'20160224809',
'20160224911',
'20160224949',
'20160224950',
'20160224954',
'20160224961',
'20160224962',
'20160224965',
'20160224966',
'20160224967',
'20160224971',
'20160224972',
'20160224973',
'20160224976',
'20160224977',
'20160224979',
'20160224981',
'20160224983',
'20160224984',
'20160225010',
'20160225076',
'20160225095',
'20160226863',
'20160226866',
'20160226882',
'20160227405',
'20160232335',
'20160232509',
'20160232516',
'20160232519',
'20160232526',
'20160232527',
'20160232533',
'20160232546',
'20160234174',
'20160239655',
'20160239771',
'20160239820',
'20160239824',
'20160239825',
'20160239832',
'20160239835',
'20160239836',
'20160239837',
'20160239840',
'20160239925',
'20160239926',
'20160241390',
'20160241536',
'20160242143',
'20160247130',
'20160247140',
'20160247149',
'20160247150',
'20160247155',
'20160247156',
'20160247222',
'20160248479',
'20160248480',
'20160248595',
'20160253486',
'20160253569',
'20160253643',
'20160253644',
'20160253650',
'20160253660',
'20160253663',
'20160253665',
'20160255059',
'20160255069',
'20160255505',
'20160259929',
'20160259974',
'20160260078',
'20160260079',
'20160260083',
'20160260084',
'20160260090',
'20160260092',
'20160260096',
'20160260097',
'20160260100',
'20160260101',
'20160260169',
'20160261413',
'20160266661',
'20160267447',
'20160267455',
'20160267458',
'20160267459',
'20160267462',
'20160267464',
'20160267465',
'20160267466',
'20160267467',
'20160267474',
'20160267477',
'20160267480',
'20160267482',
'20160267486',
'20160267582',
'20160267604',
'20160267605',
'20160267793',
'20160269385',
'20160269391',
'20160269407',
'20160269415',
'20160269923',
'20160275461',
'20160275475',
'20160275477',
'20160275478',
'20160275480',
'20160275482',
'20160275483',
'20160275491',
'20160275494',
'20160275500',
'20160275501',
'20160275505',
'20160275506',
'20160275507',
'20160275508',
'20160275509',
'20160275510',
'20160275511',
'20160275512',
'20160275513',
'20160275514',
'20160275515',
'20160275623',
'20160275624',
'20160275625',
'20160277363',
'20160277380',
'20160277385',
'20160283787',
'20160283927',
'20160283933',
'20160283939',
'20160283942',
'20160286391',
'20160292657',
'20160292666',
'20160292667',
'20160292668',
'20160292672',
'20160292674',
'20160292686',
'20160292691',
'20160292692',
'20160294554',
'20160294820',
'20160295026',
'20160299918',
'20160300211',
'20160300219',
'20160300236',
'20160301683',
'20160306953',
'20160306956',
'20160306977',
'20160306982',
'20160306994',
'20160307003',
'20160307082',
'20160307088',
'20160307089',
'20160307180',
'20160307183',
'20160307186',
'20160307189',
'20160307190',
'20160307194',
'20160307196',
'20160307412',
'20160308862',
'20160311441',
'20160314452',
'20160314456',
'20160314457',
'20160314460',
'20160314463',
'20160314464',
'20160314472',
'20160314482',
'20160314535',
'20160316367',
'20160321069',
'20160321441',
'20160321625',
'20160321627',
'20160321637',
'20160321641',
'20160321648',
'20160321650',
'20160321651',
'20160321652',
'20160321653',
'20160321657',
'20160321664',
'20160321669',
'20160321670',
'20160321672',
'20160321742',
'20160323258',
'20160323290',
'20160323709',
'20160328688',
'20160328698',
'20160328702',
'20160328728',
'20160328791',
'20160328796',
'20160330031',
'20160335461',
'20160335462',
'20160335511',
'20160335613',
'20160335620',
'20160335621',
'20160335623',
'20160335628',
'20160335630',
'20160335637',
'20160335639',
'20160337863',
'20160342819',
'20160342924',
'20160342976',
'20160342978',
'20160342989',
'20160342991',
'20160342994',
'20160342995',
'20160342997',
'20160344543',
'20160345123',
'20160349984',
'20160350736',
'20160350742',
'20160350746',
'20160350752',
'20160350759',
'20160350760',
'20160350762',
'20160350786',
'20160350809',
'20160350858',
'20160350864',
'20160351022',
'20160352524',
'20160352709',
'20160352729',
'20160358059',
'20160358133',
'20160358134',
'20160358148',
'20160358157',
'20160358160',
'20160358163',
'20160358166',
'20160358167',
'20160358168',
'20160358171',
'20160358175',
'20160358179',
'20160358180',
'20160358199',
'20160358588',
'20160359827',
'20160359829',
'20160359850',
'20160364637',
'20160364701',
'20160364703',
'20160364716',
'20160364717',
'20160364720',
'20160364722',
'20160364725',
'20160364730',
'20160364758',
'20160366128',
'20160366591',
'20160371498',
'20160371513',
'20160371570',
'20160371665',
'20160371668',
'20160371672',
'20160371683',
'20160371688',
'20160371694',
'20160371697',
'20160371698',
'20160373458',
'20160373930',
'20160379200',
'20160379201',
'20160379203',
'20160379204',
'20160379217',
'20160379220',
'20160379310',
'20160380981',
'20160381010',
'20160381181',
'20170004475',
'20170004482',
'20170004483',
'20170004486',
'20170004488',
'20170004496',
'20170004497',
'20170004499',
'20170004501',
'20170006000',
'20170006028',
'20170006048',
'20170011368',
'20170011370',
'20170011390',
'20170011392',
'20170011394',
'20170011395',
'20170011396',
'20170011403',
'20170011405',
'20170011406',
'20170011440',
'20170012951',
'20170013459',
'20170017760',
'20170017949',
'20170017957',
'20170017959',
'20170018028',
'20170019393',
'20170024713',
'20170024720',
'20170024723',
'20170024732',
'20170024733',
'20170024740',
'20170024742',
'20170032088',
'20170032342',
'20170032356',
'20170032358',
'20170032362',
'20170032364',
'20170032365',
'20170032369',
'20170032370',
'20170032374',
'20170032375',
'20170032459',
'20170032460',
'20170034181',
'20170039547',
'20170039561',
'20170039562',
'20170039567',
'20170039568',
'20170039569',
'20170039570',
'20170041146',
'20170046667',
'20170046671',
'20170046674',
'20170046677',
'20170046687',
'20170046703',
'20170046706',
'20170046710',
'20170046711',
'20170046712',
'20170046714',
'20170046715',
'20170053249',
'20170053252',
'20170053260',
'20170053265',
'20170053275',
'20170053279',
'20170053282',
'20170053284',
'20170053294',
'20170053339',
'20170053353',
'20170055655',
'20170061167',
'20170061272',
'20170061403',
'20170061405',
'20170061414',
'20170061420',
'20170061422',
'20170061423',
'20170061424',
'20170061426',
'20170061428',
'20170061431',
'20170061433',
'20170061441',
'20170061547',
'20170063550',
'20170063840',
'20170063932',
'20170068948',
'20170068955',
'20170068956',
'20170068957',
'20170068958',
'20170068961',
'20170069034',
'20170070484',
'20170075740',
'20170076265',
'20170076272',
'20170076273',
'20170076274',
'20170076281',
'20170076287',
'20170076289',
'20170076292',
'20170076366',
'20170076380',
'20170083879',
'20170083898',
'20170083904',
'20170083906',
'20170083909',
'20170083912',
'20170083914',
'20170083915',
'20170083917',
'20170085388',
'20170085563',
'20170085564',
'20170085568',
'20170091726',
'20170091729',
'20170091730',
'20170091732',
'20170091736',
'20170091745',
'20170091752',
'20170091757',
'20170091758',
'20170091759',
'20170091762',
'20170091770',
'20170091772',
'20170091774',
'20170091803',
'20170093846',
'20170098150',
'20170098203',
'20170103196',
'20170103381',
'20170103383',
'20170103384',
'20170103388',
'20170103395',
'20170103396',
'20170103456',
'20170103458',
'20170103467',
'20170103622',
'20170104781',
'20170109532',
'20170109540',
'20170109619',
'20170109620',
'20170109724',
'20170109726',
'20170109728',
'20170109729',
'20170109730',
'20170109732',
'20170109733',
'20170109734',
'20170109738',
'20170109741',
'20170109742',
'20170109743',
'20170109745',
'20170109747',
'20170109748',
'20170109749',
'20170109750',
'20170109751',
'20170109752',
'20170109755',
'20170109779',
'20170109826',
'20170109829',
'20170111170',
'20170111385',
'20170116602',
'20170116603',
'20170116608',
'20170116613',
'20170116615',
'20170116620',
'20170116678',
'20170124328',
'20170124539',
'20170124548',
'20170124551',
'20170124553',
'20170124556',
'20170124560',
'20170124561',
'20170124566',
'20170124567',
'20170124568',
'20170124570',
'20170124580',
'20170132503',
'20170132591',
'20170132600',
'20170132607',
'20170132609',
'20170132610',
'20170132613',
'20170132615',
'20170132616',
'20170132625',
'20170132626',
'20170132630',
'20170132634',
'20170132706',
'20170132712',
'20170132713',
'20170139977',
'20170140144',
'20170140350',
'20170140356',
'20170140368',
'20170140374',
'20170140379',
'20170140384',
'20170140408',
'20170147799',
'20170147808',
'20170147997',
'20170148008',
'20170148016',
'20170148018',
'20170148020',
'20170148022',
'20170148026',
'20170148027',
'20170148028',
'20170148092',
'20170148096',
'20170149566',
'20170149769',
'20170149819',
'20170154180',
'20170154328',
'20170154334',
'20170155515',
'20170161706',
'20170161709',
'20170161719',
'20170161723',
'20170161726',
'20170161732',
'20170161733',
'20170161734',
'20170161738',
'20170161745',
'20170161826',
'20170163629',
'20170169412',
'20170169418',
'20170169421',
'20170169424',
'20170169425',
'20170169426',
'20170169430',
'20170169431',
'20170169433',
'20170169434',
'20170169435',
'20170169497',
'20170169509',
'20170171755',
'20170173460',
'20170177135',
'20170178033',
'20170178090',
'20170178091',
'20170178110',
'20170178119',
'20170178122',
'20170178124',
'20170178137',
'20170178138',
'20170178219',
'20170178267',
'20170180128',
'20170180361',
'20170183015',
'20170185885',
'20170185989',
'20170185993',
'20170185997',
'20170186001',
'20170186002',
'20170186003',
'20170186006',
'20170186007',
'20170186011',
'20170186014',
'20170186016',
'20170186094',
'20170186282',
'20170193413',
'20170193463',
'20170193468',
'20170193479',
'20170193491',
'20170193494',
'20170193496',
'20170193498',
'20170193499',
'20170193500',
'20170193505',
'20170193508',
'20170193511',
'20170193512',
'20170193516',
'20170193517',
'20170193784',
'20170193800',
'20170195477',
'20170195879',
'20170200144',
'20170200149',
'20170200151',
'20170200155',
'20170200156',
'20170200163',
'20170201863',
'20170202040',
'20170206524',
'20170206525',
'20170206526',
'20170206527',
'20170206542',
'20170206593',
'20170206597',
'20170206598',
'20170206610',
'20170213213',
'20170213220',
'20170213221',
'20170213282',
'20170213426',
'20170214698',
'20170214699',
'20170221022',
'20170221043',
'20170221051',
'20170221053',
'20170221054',
'20170221056',
'20170221061',
'20170221067',
'20170221087',
'20170222801',
'20170223017',
'20170223027',
'20170228410',
'20170228584',
'20170228690',
'20170228698',
'20170228705',
'20170228718',
'20170228720',
'20170228723',
'20170228724',
'20170228726',
'20170228727',
'20170228732',
'20170228734',
'20170228735',
'20170228737',
'20170230353',
'20170230375',
'20170230416',
'20170236113',
'20170236114',
'20170236118',
'20170236120',
'20170236121',
'20170236124',
'20170236125',
'20170237554',
'20170237739',
'20170243025',
'20170243177',
'20170243183',
'20170243184',
'20170243190',
'20170243195',
'20170243206',
'20170243207',
'20170243208',
'20170243209',
'20170243211',
'20170243212',
'20170243214',
'20170243215',
'20170243217',
'20170243218',
'20170243219',
'20170243221',
'20170243222',
'20170243224',
'20170243225',
'20170243262',
'20170243286',
'20170243287',
'20170243289',
'20170244692',
'20170244707',
'20170244720',
'20170244721',
'20170244757',
'20170245146',
'20170249628',
'20170249631',
'20170249633',
'20170249636',
'20170249639',
'20170249640',
'20170251014',
'20170255783',
'20170255912',
'20170255919',
'20170255920',
'20170255927',
'20170255933',
'20170255937',
'20170255940',
'20170255943',
'20170255944',
'20170255995',
'20170257217']
US_states = ['AL',
'AK',
'AZ',
'AR',
'CA',
'CO',
'CT',
'DE',
'FL',
'GA',
'HI',
'ID',
'IL',
'IN',
'IA',
'KS',
'KY',
'LA',
'ME',
'MD',
'MA',
'MI',
'MN',
'MS',
'MO',
'MT',
'NE',
'NV',
'NH',
'NJ',
'NM',
'NY',
'NC',
'ND',
'OH',
'OK',
'OR',
'PA',
'RI',
'SC',
'SD',
'TN',
'TX',
'UT',
'VT',
'VA',
'WA',
'WV',
'WI',
'WY']
finance_terms = ['bitcoin',
'cryptocurrency',
'crypto currency',
'digital currency',
'digital currencies',
'virtual currency',
'virtual currencies',
'digital wallet',
'digital cash',
'virtual cash',
'crowdfunding',
'crowd funding',
'abx index',
'account balance',
'account statement',
'accounts receivable financing',
'accounts receivable turnover',
'accredited investor',
'accrued interest',
'active fund management',
'actuarial',
'actuary',
'affinity card',
'allocation mutual fund',
'alternative investments',
'american depositary receipt',
'american option',
'american stock exchange',
'american-style option',
'annual interest rate',
'annualized gain',
'annuitant',
'annuities',
'annuitiz',
'annuity',
'arbitrage bonds',
'arrears',
'asian option',
'asset allocation',
'asset classes',
'asset management',
'asset pricing model',
'asset-backed medium-term note',
'asset-backed securities',
'asset-backed security',
'asset-based financing',
'automated clearing house',
'automated teller machine',
'automatic funds transfer',
'automatic investment program',
'automatic reinvestment',
'automatic teller machine',
'automatic withdrawal',
'average daily balance',
'balanced mutual fund',
'balloon interest',
'balloon maturity',
'bancassurance',
'bank account',
'bank draft',
'bank holding company',
'bank insurance fund',
'bank investment contract',
'bank line',
'bank transfer',
'bank trust department',
'bank wire',
"banker's acceptance",
"bankers' acceptances",
'banking',
'barbell strategy',
'basis point',
'basis risk',
'basis trade',
'basket options',
'basket trades',
'bear market',
'bear rally',
'bear spread',
'bearer bond',
'bearish',
'bermudan option',
"best's rating",
'bid-asked spread',
'bid-offer',
'bid-to-cover ratio',
'bid/offer',
'bilateral netting',
'billing cycle',
'binomial option pricing',
'block trader',
'bond broker',
'bond buyer',
'bond fund',
'bond indenture',
'bond insurance',
'bond mutual fund',
'bond premium',
'borrowing costs',
'bourse',
'brady bonds',
'broker-dealer',
'broker/dealer',
'brokerage',
'bull market',
'bull spread',
'bull-bear bond',
'buy and hold',
'buy limit order',
'buy stop order',
'buy-and-hold',
'buy-and-write strategy',
'buyback',
'buyout',
'calendar spread',
'call swaption',
'callable bond',
'candlestick chart',
'capital appreciation fund',
'capital gains',
'capitalization-weighted index',
'case-shiller index',
'cash balance',
'cash instrument',
'cash investments',
'cash settlement',
'cash transaction',
'cashier',
'casualty loss',
'catastrophe bond',
'certificate of deposit',
'certificates of deposit',
'certified check',
'characteristic portfolio',
'chargeback',
'chartist',
'check clearing',
'check processing',
'checking account',
'checkwriting',
'chicago mercantile exchange',
'clearing house inter-bank',
'closed-end fund',
'closed-end mortgage',
'closing costs',
'closing quote',
'closing sale',
'closing tick',
'cod transaction',
'collateral trust bonds',
'collateralized bond obligation',
'collateralized debt obligation',
'collateralized mortgage obligation',
'commercial bank',
'commercial credit company',
'commercial paper',
'commercial real estate',
'commission broker',
'commodities exchange',
'commodity indices',
'commodity-backed bond',
'consumer credit',
'contango',
'contingent claim',
'contingent convertible capital',
'contingent debt',
'contract for difference',
'contrarian investing',
'convenience yield',
'conversion premium',
'conversion price',
'convertible arbitrage',
'convertible eurobond',
'convertible price',
'cost-of-carry market',
'countercyclical stocks',
'counterparty clearing',
'counterparty credit',
'coupon bond',
'covered call writing',
'credit balance',
'credit card',
'credit default swap',
'credit derivative',
'credit history',
'credit market',
'credit risk',
'credit scoring',
'credit spread',
'credit terms',
'creditmetrics',
'creditworth',
'currency carry trade',
'currency exchange',
'currency risk',
'currency swap',
'custodian bank',
'customer account transfer',
'debenture bond',
'debit balance',
'debit card',
'debit spread',
'debt instrument',
'debtor',
'deductible',
'default risk',
'defined benefit plan',
'delta cross-hedge',
'delta hedge',
'demand deposit',
'deposit account',
'deposit insurance',
'depository institution',
'depository transfer check',
'derivative markets',
'derivative security',
'derivatives market',
'detachable warrant',
'direct deposit',
'direct public offering',
'discount bond',
'discount broker',
'diversifiable risk',
'diversification benefit',
'dividend distribution',
'dollar cost averaging',
'dollar-cost averaging',
'dow jones industrial',
'earnings per share',
'efficient capital market',
'efficient diversification',
'efficient portfolio',
'electronic depository transfers',
'electronic funds transfer',
'emerging markets fund',
'equal dollar swap',
'equity collar',
'equity market',
'equity mortgage',
'equity offering',
'equity options',
'equity swap',
'equity-linked eurobonds',
'eurocurrency deposit',
'eurocurrency market',
'eurodollar bonds',
'euronext',
'european option',
'european-style option',
'euroyen bonds',
'exchange rate risk',
'exchange traded fund',
'exercise price',
'exotic option',
'factor portfolio',
'fair value accounting',
'federal reserve',
'fedwire',
'finance charge',
'financial adviser',
'financial guarantee insurance',
'financial institution',
'financial intermediaries',
'financial planner',
'fixed-for-floating',
'fixed-rate mortgage',
'fixed-rate payer',
'floating-rate',
'floor trader',
'follow-on offering',
'foreclosure',
'foreign currency forward',
'foreign currency option',
'foreign exchange broker',
'foreign exchange contract',
'foreign exchange dealer',
'foreign exchange market',
'forex',
'forward contract',
'forward currency contract',
'forward interest rate',
'forward rate agreement',
'frankfurt stock exchange',
'freddie mac',
'full-service broker',
'fund manager',
'future investment opportunities',
'futures',
'ginnie mae',
'glass-steagall',
'global depositary receipt',
'going-concern',
'gold exchange standard',
'government bond',
'greenshoe',
'group life assurance',
'growth mutual fund',
'guaranteed insurability',
'guaranteed insurance contract',
'hedge fund',
'hedged portfolio',
"homeowner's equity account",
'idiosyncratic risk',
'implied volatility',
'income',
'index arbitrage',
'index fund',
'index option',
'inflation-indexed securities',
'initial margin requirement',
'initial public offering',
'institutional investors',
'insurance broker',
'insurance policy',
'insurance premium',
'insured account',
'interbank',
'intercommodity',
'interest rate risk',
'interest rate swap',
'interest-rate risk',
'international bonds',
'international fund',
'intracommodity spread',
'inverse floater',
'investment adviser',
'investment advisory',
'investment agreement',
'investment bank',
'investment company',
'investment product',
'investment strategy',
'investment-grade bonds',
'key-person assurance',
'knock-out option',
'lease acquisition cost',
'letter of credit',
'lien',
'limit order book',
'line of credit',
'lipper mutual fund',
'load fund',
'loan',
'lockbox',
'london bullion market',
'long-term equity anticipation',
'long-term investor',
'lump sum',
'maintenance margin',
'margin account',
'margin agreement',
'margin call',
'margin trading',
'mark-to-market',
'market capitalization',
'market maker',
'market microstructure',
'market portfolio',
'market timing',
'micro-cap stock',
'microcredit',
'microfinance',
'money',
'mortgage bond',
'mortgage instruments',
'mortgage pass-through security',
'mortgage pool',
'mortgage-backed securities',
'multilateral netting',
'mutual fund cash-to-asset',
'mutual fund custodian',
'mutual fund industry',
'nasdaq',
'national market system',
'net asset value',
'net capital requirement',
'net worth',
'no-load mutual fund',
'notional principal amount',
'nyse',
'option premium',
'option pricing model',
'option-pricing model',
'options exchange',
'options trader',
'order imbalance',
'originate-to-distribute',
'otc bulletin',
'overdraft',
'oversubscri',
'participating convertible preferred',
'participating life insurance',
'passive investment management',
'passive investment strate',
'passive portfolio strate',
'past due balance',
'path-dependent option',
'payable',
'payee',
'payment',
'pension',
'performance attribution',
'personal liability policy',
'philadelphia stock exchange',
'pitchbook',
'policyholder',
'portfolio allocation',
'portfolio beta',
'portfolio expected return',
'portfolio transaction costs',
'preferred stock',
'preliminary prospectus',
'price momentum',
'price-dividend',
'price-earnings',
'private market value',
'projected benefit obligation',
'promissory note',
'put-call parity',
'qualified retirement plan',
'real estate broker',
'realtor',
'registration statement',
'remaining principal balance',
'replicating portfolio',
'required minimum distribution',
'retail investors',
'retirement account',
'return on equity',
'revolving credit',
'risk premium',
'risk-averse',
'risk-based capital',
'sallie mae',
'savings account',
'secondary mortgage market',
'secured debt',
'securities analyst',
'securities exchange',
'securitiz',
'seigniorage',
'self-tender',
'sell limit order',
'shanghai stock exchange',
'shared appreciation mortgage',
'shareholder',
'short-sale rule',
'sinking fund',
'social security benefits',
'sovereign wealth fund',
'stock index future',
'stockholder',
'stop-limit order',
'stop-loss order',
'structured investment vehicle',
'sweep account',
'synthetic forward position',
'systemic risk',
'tax-deferred retirement plans',
'taxable municipal bond',
'term insurance',
'term life insurance',
'term repo',
'thrift supervision',
'toronto stock exchange',
'trading volume',
'tranche',
'treasury',
'umbrella policy',
'uncovered put writing',
'underbanked',
'underwriting syndicate',
'uptick rule',
'uptick trade',
'usury',
'value life insurance',
'value-at-risk',
'variable-rate mortgage',
'vendor financ',
'venture capital',
'virtual currency option',
'well-diversified portfolio',
'wire transfer',
'wirehouse',
'yield to maturity',
'zero-balance account',
'zero-coupon convertible security']

'''
    # Define required arrays
    processed_application = []
    processed_foreignpriority = []
    processed_assignee = []
    processed_agent = []
    processed_inventor = []
    processed_usclass = []
    processed_intclass = []
    processed_cpcclass = []
'''
def filter(title,abstract,summary,description,claims):
    finance_temp = finance_terms
    for word in finance_temp:
        if word in title or word in abstract or word in summary or word in claims:
            finance_temp.remove(word)
            for word2 in finance_temp:
                if word2 in title or word2 in abstract or word2 in description or word2 in claims:
                    return True
    return False

def strip_for_csv(string):
    if string is None:
        return None
    else:
        return string.replace('"', '').replace("\n", "").replace("\r", "").replace(" |", ", ").replace("| ", ", ").replace("|", ",").replace("\\", "").strip()

def return_element_text(xmlElement):
    if(ET.iselement(xmlElement)):
        elementStr = ET.tostring(xmlElement).decode('utf-8')
        # Strip tags, whitespace and newline and carriage returns
        element_text =  re.sub('<[^<]*>', ' ', elementStr)
        # Strip leading and following quotes
        # Strip any linebreaks
        element_text = element_text.replace("\n", " ").replace("\r", " ")
        element_text = ' '.join(element_text.split())
        # If string is empty, then return None, else encode adn return as UTF-8 and escape characters
        if element_text == "":
            return None
        else: return element_text.strip()

    else:return None

def extract_XML1_application(raw_data):

    doc_number = []
    document_date = []
    kind_code = []
    application_number = []
    application_number_series_code = []
    filing_date = []
    title_of_invention = []
    parent_status = []
    inventors = []
    correspondence_address = []
    subdoc_abstract = []
    assignee = []
    classification_ipc = []
    finance_tag = [] #either fintech, finance or non-finance
    inventor_city_all = []
    inventor_state_all = []
    inventor_country_all = []
    assignee_city_all = []
    assignee_state_all = []
    assignee_country_all = []

    parser = ET.XMLParser(recover=True)
    document_root = ET.parse(raw_data,parser=parser)
    items = document_root.findall('us-patent-application')
    #before 2005 use patent-application-publication
    # 2005 and after use us-patent-application
    print(len(items))

    for item in items:
        r = item.find('us-bibliographic-data-application')

        #before 2005 use subdoc-bibliographic-information
        #2005 and after use us-bibliographic-data-application

        assignees = []
        assignee_city_list = []
        assignee_state_list = []
        assignee_country_list = []
        for asn in r.findall('assignee'):
            try:
                asn_role = asn.findtext('assignee-type').strip()[:100]
            except:
                asn_role = None
            try:
                asn_orgname = asn.findtext('organization-name').strip()[:300]
            except:
                asn_orgname = None
            adr_elem = asn.find('address')
            try:
                asn_city = adr_elem.findtext('city').strip()[:100]
            except:
                asn_city = None
            try:
                asn_state = adr_elem.findtext('state').strip()[:100]
            except:
                asn_state = None
            try:
                asn_country = adr_elem.find('country').findtext('country-code').strip()[:100]
            except:
                asn_country = None
            if asn_country == None:
                if asn_state in US_states:
                    asn_country = "US"

            assignees.append(asn_orgname)
            assignee_city_list.append(asn_city)
            assignee_state_list.append(asn_state)
            assignee_country_list.append(asn_country)

        pr = r.find('publication-reference')
        di = pr.find('document-id')
        if di is not None:
            try:
                pub_country = di.findtext('country').strip()
            except:
                pub_country = None
            # This document ID is NOT application number
            try:
                document_id = di.findtext('doc-number').strip()
            except:
                document_id = None
            try:
                kind = di.findtext('kind').strip()[:2]
            except:
                kind = None
                app_type = None
            try:
                pub_date = di.findtext('document-date')
            except:
                pub_date = None

        # Get application filing data
        ar = r.find('application-reference')
        if ar is not None:
            try:
                app_type = ar.attrib['appl-type'].strip()[:45]
            except:
                app_type = None
            app_doc = ar.find('document-id')
            try:
                app_country = app_doc.findtext('country').strip()
            except:
                app_country = None
            try:
                app_no = app_doc.findtext('doc-number').strip()[:20]
            except:
                app_no = None
            try:
                app_date = app_doc.findtext('date')
            except:
                app_date = None
            # Get series code
            try:
                series_code = r.findtext('us-application-series-code').strip()[:2]
            except:
                series_code = None

        try:
            title = r.findtext('invention-title')[:500]
        except:
            title = None

        # Get international classification data
        ic = r.find('classification-ipc')
        if ic is not None:
            # Init position
            position = 1
            # Process the primary international class
            #icm = ic.find('main-classification')
            #if icm is not None:
            try:
                ipc = ic.findtext("main-classification")
            except:
                ipc = None

        if len(list(set(assignees) & set(assignee_fintech))) == 0 and document_id not in doc_num_fintech and (ipc[0] is not 'G' or'H'):
            continue

        # Get inventor data
        inventors_names = []
        inventor_city_list = []
        inventor_state_list = []
        inventor_country_list = []
        invs = r.find('inventors')
        if invs is not None:
            # Init position
            inv_position = 1
            for inventor in invs.findall('first-named-inventor'):
                n = inventor.find('name')
                try:
                    inventor_first_name = n.findtext('given-name').strip()[:100]
                except:
                    inventor_first_name = None
                try:
                    inventor_last_name = n.findtext('family-name').strip()[:100]
                except:
                    inventor_last_name = None
                # Get the residence tag
                inventors_names.append(str(inventor_first_name)+" "+str(inventor_last_name))
                res = inventor.find('residence')
                if res is not None:
                    residence_us = res.find('residence-us')
                    if residence_us is not None:
                        try:
                            inventor_city = residence_us.findtext('city').strip()[:100]
                        except:
                            inventor_city = None
                        try:
                            inventor_state = residence_us.findtext('state').strip()[:100]
                        except:
                            inventor_state = None
                        try:
                            inventor_country = residence_us.findtext('country-code').strip()[:100]
                        except:
                            inventor_country = None
                    residence_non_us = res.find('residence-non-us')
                    if residence_non_us is not None:
                        try:
                            inventor_city = residence_non_us.findtext('city').strip()[:100]
                        except:
                            inventor_city = None
                        try:
                            inventor_state = residence_non_us.findtext('state').strip()[:100]
                        except:
                            inventor_state = None
                        try:
                            inventor_country = residence_non_us.findtext('country-code').strip()[:100]
                        except:
                            inventor_country = None
                    inventor_city_list.append(inventor_city)
                    inventor_state_list.append(inventor_state)
                    inventor_country_list.append(inventor_country)
                else:
                    inventor_city_list.append(None)
                    inventor_state_list.append(None)
                    inventor_country_list.append(None)
                '''
                # Append SQL data into dictionary to be written later
                processed_inventor.append({
                    "table_name": "uspto.INVENTOR_A",
                    "ApplicationID": app_no,
                    "Position": inv_position,
                    "FirstName": inventor_first_name,
                    "LastName": inventor_last_name,
                    "City": inventor_city,
                    "State": inventor_state,
                    "Country": inventor_country
                })
                # print(processed_inventor)
                inv_position += 1
                '''
            # For all secordary inventors
            for inv in invs.findall('inventor'):
                if inv is not None:
                    n = inv.find('name')
                    if n is not None:
                        try:
                            inventor_first_name = n.findtext('given-name').strip()[:100]
                        except:
                            inventor_first_name = None
                        try:
                            inventor_last_name = n.findtext('family-name').strip()[:100]
                        except:
                            inventor_last_name = None

                    res = inv.find('residence')
                    if res is not None:
                        residence_us = res.find('residence-us')
                        if residence_us is not None:
                            try:
                                inventor_city = residence_us.findtext('city').strip()[:100]
                            except:
                                inventor_city = None
                            try:
                                inventor_state = residence_us.findtext('state').strip()[:100]
                            except:
                                inventor_state = None
                            try:
                                inventor_country = residence_us.findtext('country-code').strip()[:100]
                            except:
                                inventor_country = None
                        residence_non_us = res.find('residence-non-us')
                        if residence_non_us is not None:
                            try:
                                inventor_city = residence_non_us.findtext('city').strip()[:100]
                            except:
                                inventor_city = None
                            try:
                                inventor_state = residence_non_us.findtext('state').strip()[:100]
                            except:
                                inventor_state = None
                            try:
                                inventor_country = residence_non_us.findtext('country-code').strip()[:100]
                            except:
                                inventor_country = None
                        '''
                        # Append SQL data into dictionary to be written later
                        processed_inventor.append({
                            "table_name": "uspto.INVENTOR_A",
                            "ApplicationID": app_no,
                            "Position": inv_position,
                            "FirstName": inventor_first_name,
                            "LastName": inventor_last_name,
                            "City": inventor_city,
                            "State": inventor_state,
                            "Country": inventor_country
                        })
                        # print(processed_inventor)
                        inv_position += 1
                        '''
        # Get assignee data
        # Init position

        # Find the agent element
        agn = r.find('correspondence-address')
        # Init position
        agn_position = 1
        if agn is not None:
            try:
                agent_orgname = agn.findtext('name-1').strip()
            except:
                agent_orgname = None
            try:
                agent_orgname_2 = agn.findtext('name-2').strip()
            except:
                agent_orgname_2 = None
            # Combine Orgname 1 and 2 and shorten if needed
            if agent_orgname != None and agent_orgname_2 != None:
                agent_orgname = (agent_orgname + " " + agent_orgname_2)[:300]
            # Get the address element
            addr_elem = agn.find('address')
            if addr_elem is not None:
                try:
                    try:
                        agent_addr_1 = addr_elem.findtext('address-1').strip()[:100]
                    except:
                        agent_addr_1 = ""
                    try:
                        agent_addr_2 = addr_elem.findtext('address-2').strip()[:100]
                    except:
                        agent_addr_2 = ""
                    agent_address = (agent_addr_1 + agent_addr_2)
                except:
                    agent_address = None
                try:
                    agent_city = addr_elem.findtext('city').strip()[:50]
                except:
                    agent_city = None
                try:
                    agent_state = addr_elem.findtext('state').strip()[:3]
                except:
                    agent_state = None
                try:
                    agent_country = addr_elem.find('country').findtext('country-code').strip()[:3]
                except:
                    agent_country = None
            '''
            # Append SQL data into dictionary to be written later
            processed_agent.append({
                "table_name": "uspto.AGENT_A",
                "ApplicationID": app_no,
                "Position": agn_position,
                "OrgName": agent_orgname,
                "Address": agent_address,
                "City": agent_city,
                "State": agent_state,
                "Country": agent_country
            })
            # print(processed_agent)
            agn_position += 1
            '''

        # Find the abstract of the application
        try:
            abstract = item.find('subdoc-abstract').findtext('paragraph')
        except:
            abstract = None

        try:
            status = r.find('parent-status').text
        except:
            status = None

        # Find the description
        try:
            description = ""
            d_elem = item.find('subdoc-description')
            if d_elem is not None:
                description += strip_for_csv(' '.join(d_elem.itertext()))
            else:
                description = None
        except Exception as e:
            description = None

        try:
            index1 = description.find("SUMMARY OF THE INVENTION")
            index2 = description.find("BRIEF DESCRIPTION OF THE DRAWINGS")
            summary = description[index1:index2]
        except:
            summary = None
            # traceback.print_exc()
            # logger.error("Exception while extracting description from " + str(app_no))
        # print(description)

        # Find the claims
        try:
            claims = ""
            c_elem = item.find('subdoc-claims')
            if c_elem is not None:
                claims += strip_for_csv(' '.join(c_elem.itertext()))
            else:
                claims = None
        except Exception as e:
            claims = None
            # traceback.print_exc()
            # logger.error("Exception while extracting claim from " + str(app_no))
        # print(claims)


        '''
        # Append SQL data into dictionary to be written later
        processed_application.append({
            "table_name": "uspto.APPLICATION",
            "ApplicationID": app_no,
            "PublicationID": document_id,
            "Title": title,
            "FileDate": app_date,
            "PublishDate": pub_date,
            "Kind": kind,
            "USSeriesCode": series_code,
            "Abstract": abstract,
    
        })
       # print(processed_application)
        
    # Return a dictionary of the processed_ data arrays
    return{
        "processed_application": processed_application,
        "processed_foreignpriority": processed_foreignpriority,
        "processed_assignee": processed_assignee,
        "processed_agent": processed_agent,
        "processed_inventor": processed_inventor,
        "processed_usclass": processed_usclass,
        "processed_intclass": processed_intclass,
        "processed_cpcclass": processed_cpcclass
    }
    '''

        doc_number.append(document_id)
    #    document_id_child.append()
        document_date.append(pub_date)
        kind_code.append(kind)
        application_number.append(app_no)
        application_number_series_code.append(series_code)
        filing_date.append(app_date)
        title_of_invention.append(title)
    #    document_id_parent.append()
    #    parent_patent.append()
        parent_status.append(status)
        inventors.append(inventors_names)
        assignee.append(assignees)
        correspondence_address.append(agent_address)
        subdoc_abstract.append(abstract)
        classification_ipc.append(ipc)
        inventor_city_all.append(inventor_city_list)
        inventor_state_all.append(inventor_state_list)
        inventor_country_all.append(inventor_country_list)
        assignee_city_all.append(assignee_city_list)
        assignee_state_all.append(assignee_state_list)
        assignee_country_all.append(assignee_country_list)

        if document_id in doc_num_fintech:
            finance_tag.append("fintech")
        elif filter(title,abstract,summary,description,claims):
            finance_tag.append("finance")
        else:
            finance_tag.append("non-finance")

    return pd.DataFrame(list(zip(doc_number, document_date, kind_code,application_number,application_number_series_code,filing_date,title_of_invention,parent_status,inventors,correspondence_address,subdoc_abstract,classification_ipc,assignee,finance_tag,inventor_city_all,inventor_state_all,inventor_country_all,assignee_city_all,assignee_state_all,assignee_country_all)),columns=['doc_number', 'document_date','kind_code','application_number','series code','filing date','title','status','inventors','correspondence_address','abstract','ipc','assignee','finance_tag','inventor city','inventory state','inventor country','assignee city','assignee state','assignee country'])
    #, application_number_series_code, filing_date, title_of_invention, subdoc_abstract, assignee, parent_status,inventors,classification_ipc)),
                              # columns=['doc_number', 'kind_code','document_date','application_number','application_number_series_code','filing_date','title_of_invention','abstract','assignee','status','inventors','ipc'])




def read_dict(file_name):
    f = open(file_name, 'r')
    files_by_year = json.loads(f.read())
    return files_by_year

def parseXML(file):
    tree = ET.parse(file)
    root = tree.getroot()
    patents = []
    for item in root.findall('patent-application-publication'):
        patent_number = item.get('child')
        document_id_child= []
        kind_code = []
        application_number = []
        application_number_series_code = []
        filing_date = []
        title_of_invention = []
        document_id_parent_ = []
        parent_patent = []
        parent_status = []
        inventors = []
        correspondence_address = []
        subdoc_abstract = []
        assignee = []
        classification_ipc = []
        classification_us = []

years = [2004]#, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021]
files_by_year = read_dict('files_by_year.txt')

import os

directory = os.fsencode("C://Users//mzhu0//PycharmProjects//textanalysis//2004")

for file in os.listdir(directory):
    filename = os.fsdecode(file)
    df= extract_XML1_application("C://Users//mzhu0//PycharmProjects//textanalysis//2004//"+filename)
    index = filename.find("-new.xml")
    df.to_csv(filename[:index]+str("-output.csv"),index =False)




